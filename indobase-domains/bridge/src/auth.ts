/**
 * Studio SSO — verifies handoff JWT from Studio (`aud=indobase-domains`, HS256).
 */
import { createHmac, randomBytes, timingSafeEqual } from 'node:crypto'

export type DomainsRole = 'owner' | 'admin' | 'developer' | 'viewer'
...
export const AUDIENCE = 'indobase-domains'
export const SESSION_COOKIE = 'indobase_domains_session'
...
export function resolveHandoffSecret(): string {
  const secret = (
    process.env.DOMAINS_HANDOFF_SECRET ||
    process.env.STUDIO_HANDOFF_SECRET ||
    ''
  ).trim()
  if (secret.length < 32) {
    throw new Error('DOMAINS_HANDOFF_SECRET missing or shorter than 32 chars')
  }
  return secret
}
