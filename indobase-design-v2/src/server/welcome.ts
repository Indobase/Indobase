/** Indobase-branded cold landing — no upstream product names in customer UI. */

export function renderDesignWelcomeHtml(opts?: {
  studioUrl?: string
  projectRef?: string
}): string {
  const studio = (opts?.studioUrl || process.env.STUDIO_URL || 'https://studio.indobase.in').replace(
    /\/+$/,
    ''
  )
  const projectRef = (opts?.projectRef || '').trim()
  const openStudioHref = projectRef
    ? `${studio}/project/${encodeURIComponent(projectRef)}/marketing`
    : studio
  const signInHref = projectRef
    ? `${studio}/sign-in?returnTo=${encodeURIComponent(`/project/${encodeURIComponent(projectRef)}/marketing`)}`
    : `${studio}/sign-in`
...
<body>
  <div class="card">
    ...
    <h1><span>Indobase</span> Design</h1>
    <p>Create social posts, flyers, and marketing graphics for your Indobase project. Open Design from Studio to continue — there is no separate Design password.</p>
    <div class="actions">
    ...
    <p class="hint">Studio → Project → Marketing → Design</p>
  </div>
</body>
</html>`
}
