...
app.post('/api/billing-token', async (req, res) => {
  ...
  const r = await fetch(
    `https://payments.indobase.in/api/v1/customers/${customerId}/portal-token`,
    {
      method: 'POST',
      headers: {
...
Every widget shows a small **Powered by Indobase Payments** attribution by default. Hide
