...
export function validateUUID(str: string) {
  if (!UUID_REGEX.test(str)) {
    throw new Error('@indobaseinc/auth-js: Expected parameter to be UUID but is not')
  }
}
...
export function userNotAvailableProxy(): User {
  ...
  return new Proxy(proxyTarget, {
    get: (target: any, prop: string) => {
      ...
      throw new Error(
        `@indobaseinc/auth-js: client was created with userStorage option and there was no user stored in the user storage. Accessing the "${prop}" property of the session object is not supported. Please use getUser() instead.`
      )
    },
    set: (_target: any, prop: string) => {
      throw new Error(
        `@indobaseinc/auth-js: client was created with userStorage option and there was no user stored in the user storage. Setting the "${prop}" property of the session object is not supported. Please use getUser() to fetch a user object you can manipulate.`
      )
    },
    deleteProperty: (_target: any, prop: string) => {
      throw new Error(
        `@indobaseinc/auth-js: client was created with userStorage option and there was no user stored in the user storage. Deleting the "${prop}" property of the session object is not supported. Please use getUser() to fetch a user object you can manipulate.`
      )
    },
  })
}
