 ...
 * @example
 * ```ts
 * import { LockAcquireTimeoutError } from '@indobaseinc/auth-js'
 *
 ...
 * @example
 * ```ts
 * import { NavigatorLockAcquireTimeoutError } from '@indobaseinc/auth-js'
 *
 ...
 * @example
 * ```ts
 * import { ProcessLockAcquireTimeoutError } from '@indobaseinc/auth-js'
 *
...
export async function navigatorLock<R>(
  ...
  if (internals.debug) {
    console.log('@indobaseinc/auth-js: navigatorLock: acquire lock', name, acquireTimeout)
  }
  ...
  if (acquireTimeout > 0) {
    setTimeout(() => {
      ...
      if (internals.debug) {
        console.log('@indobaseinc/auth-js: navigatorLock acquire timed out', name)
      }
    }, acquireTimeout)
  }
  ...
  try {
    return await globalThis.navigator.locks.request(
      ...
      async (lock) => {
        if (lock) {
          if (internals.debug) {
            console.log('@indobaseinc/auth-js: navigatorLock: acquired', name, lock.name)
          }
          ...
          try {
            ...
            if (internals.debug) {
              console.log('@indobaseinc/auth-js: navigatorLock: released', name, lock.name)
            }
          }
        } else {
          if (acquireTimeout === 0) {
            if (internals.debug) {
              console.log('@indobaseinc/auth-js: navigatorLock: not immediately available', name)
            }
            ...
            if (internals.debug) {
              try {
                ...
                console.log(
                  '@indobaseinc/auth-js: Navigator LockManager state',
                  JSON.stringify(result, null, '  ')
                )
              } catch (e: any) {
                console.warn(
                  '@indobaseinc/auth-js: Error when querying Navigator LockManager state',
                  e
                )
              }
            }
            ...
            console.warn(
              '@indobaseinc/auth-js: Navigator LockManager returned a null lock when using #request without ifAvailable set to true, it appears this browser is not following the LockManager spec https://developer.mozilla.org/en-US/docs/Web/API/LockManager/request'
            )
    ...
    if (e?.name === 'AbortError' && acquireTimeout > 0) {
      ...
      //
      // See: https://github.com/Indobase/supabase/issues/42505
      if (internals.debug) {
        console.log(
          '@indobaseinc/auth-js: navigatorLock: acquire timeout, recovering by stealing lock',
          name
        )
      }

      console.warn(
        `@indobaseinc/auth-js: Lock "${name}" was not released within ${acquireTimeout}ms. ` +
        ...
        globalThis.navigator.locks.request(
          ...
          async (lock) => {
            if (lock) {
              if (internals.debug) {
                console.log(
                  '@indobaseinc/auth-js: navigatorLock: recovered (stolen)',
                  name,
                  lock.name
                )
              }
              ...
              try {
                ...
                if (internals.debug) {
                  console.log(
                    '@indobaseinc/auth-js: navigatorLock: released (stolen)',
                    name,
                    lock.name
                  )
                }
              ...
              console.warn(
                '@indobaseinc/auth-js: Navigator LockManager returned null lock even with steal: true'
              )
              return await fn()
            }
          }
        ...
        acquireTimeout >= 0
          ? new Promise((_, reject) => {
              timeoutId = setTimeout(() => {
                console.warn(
                  `@indobaseinc/auth-js: Lock "${name}" acquisition timed out after ${acquireTimeout}ms. ` +
