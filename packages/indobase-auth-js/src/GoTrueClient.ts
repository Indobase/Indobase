...
export default class GoTrueClient {
   ...
   * @example
   * ```ts
   * import { GoTrueClient } from '@indobaseinc/auth-js'
   *
   * const auth = new GoTrueClient({
   *   url: 'https://xyzcompany.indobase.in/auth/v1',
  ...
  > {
    ...
    switch (chain) {
      ...
      default:
        throw new Error(`@indobaseinc/auth-js: Unsupported chain "${chain}"`)
    }
  }
    ...
    if ('message' in credentials) {
      ...
      if (!isBrowser()) {
        if (typeof wallet !== 'object' || !options?.url) {
          throw new Error(
            '@indobaseinc/auth-js: Both wallet and url must be specified in non-browser environments.'
          )
        }
        ...
        if (
          ...
          throw new Error(
            `@indobaseinc/auth-js: No compatible Ethereum wallet interface on the window object (window.ethereum) detected. Make sure the user already has a wallet installed and connected for this app. Prefer passing the wallet interface object directly to signInWithWeb3({ chain: 'ethereum', wallet: resolvedUserWallet }) instead.`
          )
        }
      }
      ...
      const accounts = await resolvedWallet
        ...
        .catch(() => {
          throw new Error(
            `@indobaseinc/auth-js: Wallet method eth_requestAccounts is missing or invalid`
          )
        })

      if (!accounts || accounts.length === 0) {
        throw new Error(
          `@indobaseinc/auth-js: No accounts available. Please ensure the wallet is connected.`
        )
      }
    ...
    if ('message' in credentials) {
      ...
      if (!isBrowser()) {
        ...
        if (
          ...
          throw new Error(
            `@indobaseinc/auth-js: No compatible Solana wallet interface on the window object (window.solana) detected. Make sure the user already has a wallet installed and connected for this app. Prefer passing the wallet interface object directly to signInWithWeb3({ chain: 'solana', wallet: resolvedUserWallet }) instead.`
          )
        }
      }
      ...
      if ('signIn' in resolvedWallet && resolvedWallet.signIn) {
        ...
        if (Array.isArray(output) && output[0] && typeof output[0] === 'object') {
          ...
          throw new Error('@indobaseinc/auth-js: Wallet method signIn() returned unrecognized value')
        }
        ...
        if (
          ...
          throw new Error(
            '@indobaseinc/auth-js: Wallet method signIn() API returned object without signedMessage and signature fields'
          )
        }
      } else {
        if (
          ...
          throw new Error(
            '@indobaseinc/auth-js: Wallet does not have a compatible signMessage() and publicKey.toBase58() API'
          )
        }
        ...
        if (!maybeSignature || !(maybeSignature instanceof Uint8Array)) {
          throw new Error(
            '@indobaseinc/auth-js: Wallet signMessage() API returned an recognized value'
          )
        }
  ...
  > {
    try {
      ...
      if (actuallyExpiresIn * 1000 <= AUTO_REFRESH_TICK_DURATION_MS) {
        console.warn(
          `@indobaseinc/auth-js: Session as retrieved from URL expires in ${actuallyExpiresIn}s, should have been closer to ${expiresIn}s`
        )
      }
      ...
      if (timeNow - issuedAt >= 120) {
        console.warn(
          '@indobaseinc/auth-js: Session as retrieved from URL was issued over 120s ago, URL could be stale',
          issuedAt,
          expiresAt,
          timeNow
        )
        ...
        console.warn(
          '@indobaseinc/auth-js: Session as retrieved from URL was issued in the future? Check the device clock for skew',
          issuedAt,
          expiresAt,
          timeNow
        )
