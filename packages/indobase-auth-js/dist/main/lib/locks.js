...
async function navigatorLock(name, acquireTimeout, fn) {
    if (exports.internals.debug) {
        console.log('@indobase/auth-js: navigatorLock: acquire lock', name, acquireTimeout);
    }
    ...
    if (acquireTimeout > 0) {
        setTimeout(() => {
            ...
            if (exports.internals.debug) {
                console.log('@indobase/auth-js: navigatorLock acquire timed out', name);
            }
        }, acquireTimeout);
    }
    ...
    try {
        return await globalThis.navigator.locks.request(name, acquireTimeout === 0
            ...
            if (lock) {
                if (exports.internals.debug) {
                    console.log('@indobase/auth-js: navigatorLock: acquired', name, lock.name);
                }
                try {
                    return await fn();
                }
                    ...
                    if (exports.internals.debug) {
                        console.log('@indobase/auth-js: navigatorLock: released', name, lock.name);
                    }
                }
            }
            else {
                if (acquireTimeout === 0) {
                    if (exports.internals.debug) {
                        console.log('@indobase/auth-js: navigatorLock: not immediately available', name);
                    }
                    ...
                    if (exports.internals.debug) {
                        try {
                            ...
                            console.log('@indobase/auth-js: Navigator LockManager state', JSON.stringify(result, null, '  '));
                        }
                        catch (e) {
                            console.warn('@indobase/auth-js: Error when querying Navigator LockManager state', e);
                        }
                    }
                    ...
                    console.warn('@indobase/auth-js: Navigator LockManager returned a null lock when using #request without ifAvailable set to true, it appears this browser is not following the LockManager spec https://developer.mozilla.org/en-US/docs/Web/API/LockManager/request');
                    return await fn();
                }
            }
        });
        ...
        if ((e === null || e === void 0 ? void 0 : e.name) === 'AbortError' && acquireTimeout > 0) {
            ...
            if (exports.internals.debug) {
                console.log('@indobase/auth-js: navigatorLock: acquire timeout, recovering by stealing lock', name);
            }
            console.warn(`@indobase/auth-js: Lock "${name}" was not released within ${acquireTimeout}ms. ` +
            ...
            return await Promise.resolve().then(() => globalThis.navigator.locks.request(name, {
                ...
                if (lock) {
                    if (exports.internals.debug) {
                        console.log('@indobase/auth-js: navigatorLock: recovered (stolen)', name, lock.name);
                    }
                    try {
                        return await fn();
                    }
                        ...
                        if (exports.internals.debug) {
                            console.log('@indobase/auth-js: navigatorLock: released (stolen)', name, lock.name);
                        }
                    }
                }
                else {
                    // This should not happen with steal: true, but handle gracefully.
                    console.warn('@indobase/auth-js: Navigator LockManager returned null lock even with steal: true');
                    return await fn();
                }
            }));
        }
            ...
            const timeoutPromise = acquireTimeout >= 0
                ? new Promise((_, reject) => {
                    timeoutId = setTimeout(() => {
                        console.warn(`@indobase/auth-js: Lock "${name}" acquisition timed out after ${acquireTimeout}ms. ` +
