        ...
        if ('message' in credentials) {
            ...
            if (!(0, helpers_1.isBrowser)()) {
                if (typeof wallet !== 'object' || !(options === null || options === void 0 ? void 0 : options.url)) {
                    throw new Error('@indobase/auth-js: Both wallet and url must be specified in non-browser environments.');
                }
                ...
                if ('ethereum' in windowAny &&
                    ...
                    throw new Error(`@indobase/auth-js: No compatible Ethereum wallet interface on the window object (window.ethereum) detected. Make sure the user already has a wallet installed and connected for this app. Prefer passing the wallet interface object directly to signInWithWeb3({ chain: 'ethereum', wallet: resolvedUserWallet }) instead.`);
                }
            }
            ...
            const accounts = await resolvedWallet
                ...
                .then((accs) => accs)
                .catch(() => {
                throw new Error(`@indobase/auth-js: Wallet method eth_requestAccounts is missing or invalid`);
            });
            if (!accounts || accounts.length === 0) {
                throw new Error(`@indobase/auth-js: No accounts available. Please ensure the wallet is connected.`);
            }
        ...
        if ('message' in credentials) {
            ...
            if (!(0, helpers_1.isBrowser)()) {
                ...
                if ('solana' in windowAny &&
                    ...
                    throw new Error(`@indobase/auth-js: No compatible Solana wallet interface on the window object (window.solana) detected. Make sure the user already has a wallet installed and connected for this app. Prefer passing the wallet interface object directly to signInWithWeb3({ chain: 'solana', wallet: resolvedUserWallet }) instead.`);
                }
            }
            const url = new URL((_a = options === null || options === void 0 ? void 0 : options.url) !== null && _a !== void 0 ? _a : window.location.href);
            if ('signIn' in resolvedWallet && resolvedWallet.signIn) {
                ...
                if ('signedMessage' in outputToProcess &&
                    ...
                    throw new Error('@indobase/auth-js: Wallet method signIn() API returned object without signedMessage and signature fields');
                }
            }
            else {
                if (!('signMessage' in resolvedWallet) ||
                    ...
                    throw new Error('@indobase/auth-js: Wallet does not have a compatible signMessage() and publicKey.toBase58() API');
                }
                message = [
                ...
                if (!maybeSignature || !(maybeSignature instanceof Uint8Array)) {
                    throw new Error('@indobase/auth-js: Wallet signMessage() API returned an recognized value');
                }
    ...
    async _getSessionFromURL(params, callbackUrlType) {
        try {
            ...
            if (actuallyExpiresIn * 1000 <= constants_1.AUTO_REFRESH_TICK_DURATION_MS) {
                console.warn(`@indobase/auth-js: Session as retrieved from URL expires in ${actuallyExpiresIn}s, should have been closer to ${expiresIn}s`);
            }
            const issuedAt = expiresAt - expiresIn;
            if (timeNow - issuedAt >= 120) {
                console.warn('@indobase/auth-js: Session as retrieved from URL was issued over 120s ago, URL could be stale', issuedAt, expiresAt, timeNow);
            }
            else if (timeNow - issuedAt < 0) {
                console.warn('@indobase/auth-js: Session as retrieved from URL was issued in the future? Check the device clock for skew', issuedAt, expiresAt, timeNow);
            }
