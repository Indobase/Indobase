...
import { DEFAULT_POOL_CONFIG, EXPORT_DOCS, GENERATE_TYPES, GENERATE_TYPES_INCLUDED_SCHEMAS, PG_CONNECTION, PG_META_HOST, PG_META_PORT, } from './constants.js';
...
if (EXPORT_DOCS) {
    ...
    const { data: materializedViews, error: materializedViewsError } = await pgMeta.materializedViews.list({ includeColumns: true });
    ...
    console.log(applyTypescriptTemplate({
        schemas: schemas.filter(({ name }) => GENERATE_TYPES_INCLUDED_SCHEMAS.length === 0 ||
