...
export default async (fastify) => {
    ...
    fastify.get('/:tableId(^\\d+):ordinalPosition', async (request, reply) => {
        if (request.params.ordinalPosition === '') {
            const { headers: { pg: connectionString }, query: { limit, offset }, params: { tableId }, } = request;
            const includeSystemSchemas = request.query.include_system_schemas === 'true';
            const pgMeta = new PostgresMeta({ ...DEFAULT_POOL_CONFIG, connectionString });
            const { data, error } = await pgMeta.columns.list({
            ...
            const { headers: { pg: connectionString }, params: { tableId, ordinalPosition: ordinalPositionWithDot }, } = request;
    ...
    fastify.patch('/:id(\\d+\\.\\d+)', async (request, reply) => {
        const connectionString = request.headers.pg;
        const pgMeta = new PostgresMeta({ ...DEFAULT_POOL_CONFIG, connectionString });
    ...
    fastify.delete('/:id(\\d+\\.\\d+)', async (request, reply) => {
        const connectionString = request.headers.pg;
