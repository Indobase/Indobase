...
export default async (fastify) => {
    fastify.get('/', async (request, reply) => {
        ...
        const excludedSchemas = request.query.excluded_schemas?.split(',').map((schema) => schema.trim()) ?? [];
        const includedSchemas = request.query.included_schemas?.split(',').map((schema) => schema.trim()) ?? [];
