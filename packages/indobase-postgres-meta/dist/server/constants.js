...
export const PG_CONNECTION = process.env.PG_META_DB_URL ||
    `postgres://${PG_META_DB_USER}:${encodeURIComponent(PG_META_DB_PASSWORD)}@${PG_META_DB_HOST}:${PG_META_DB_PORT}/${PG_META_DB_NAME}?sslmode=${PG_META_DB_SSL_MODE}`;
export const EXPORT_DOCS = process.argv[2] === 'docs' && process.argv[3] === 'export';
export const GENERATE_TYPES = process.argv[2] === 'gen' && process.argv[3] === 'types' ? process.argv[4] : undefined;
export const GENERATE_TYPES_INCLUDED_SCHEMAS = GENERATE_TYPES && process.argv[5] === '--include-schemas' ? process.argv[6]?.split(',') ?? [] : [];
