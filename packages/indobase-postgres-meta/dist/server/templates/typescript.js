...
export const apply = ({ schemas, tables, views, materializedViews, functions, types, arrayTypes, }) => {
    let output = `
  ...
  ${schemas
        ...
        return `${JSON.stringify(schema.name)}: {
          Tables: {
            ...
            ? '[_ in never]: never'
            : schemaTables.map((table) => `${JSON.stringify(table.name)}: {
                  Row: {
                    ${[
                ...table.columns
                    .sort(({ name: a }, { name: b }) => a.localeCompare(b))
                    .map((column) => `${JSON.stringify(column.name)}: ${pgTypeToTsType(column.format, types, schemas)} ${column.is_nullable ? '| null' : ''}`),
                ...schemaFunctions
                    .filter((fn) => fn.argument_types === table.name)
                    .map((fn) => `${JSON.stringify(fn.name)}: ${pgTypeToTsType(fn.return_type, types, schemas)} | null`),
            ]}
                  }
                  Insert: {
                    ${table.columns
          ...
          Views: {
            ...
            ? '[_ in never]: never'
            : schemaViews.map((view) => `${JSON.stringify(view.name)}: {
                  Row: {
                    ${view.columns
                .sort(({ name: a }, { name: b }) => a.localeCompare(b))
                .map((column) => `${JSON.stringify(column.name)}: ${pgTypeToTsType(column.format, types, schemas)} ${column.is_nullable ? '| null' : ''}`)}
          ...
          Functions: {
            ...
            return Object.entries(schemaFunctionsGroupedByName).map(([fnName, fns]) => `${JSON.stringify(fnName)}: ${fns
                .map(({ args, return_type_id, return_type_relation_id, is_set_returning_function, }) => `{
                  Args: ${(() => {
                ...
                return `{
                      ${argsNameAndType.map(({ name, type, has_default }) => `${JSON.stringify(name)}${has_default ? '?' : ''}: ${type}`)}
                    }`;
            })()}
                  Returns: (${(() => {
                ...
                const relation = [...tables, ...views].find(({ id }) => id === return_type_relation_id);
                if (relation) {
                    return `{
                        ${relation.columns
                        .sort(({ name: a }, { name: b }) => a.localeCompare(b))
          ...
          Enums: {
            ...
            ? '[_ in never]: never'
            : schemaEnums.map((enum_) => `${JSON.stringify(enum_.name)}: ${enum_.enums
          ...
          CompositeTypes: {
            ...
            ? '[_ in never]: never'
            : schemaCompositeTypes.map(({ name, attributes }) => `${JSON.stringify(name)}: {
                        ${attributes.map(({ name, type_id }) => {
                const type = types.find(({ id }) => id === type_id);
                if (type) {
                    return `${JSON.stringify(name)}: ${pgTypeToTsType(type.name, types, schemas)}`;
                }
...
const pgTypeToTsType = (pgType, types, schemas) => {
    if (pgType === 'bool') {
        return 'boolean';
    }
        ...
        if (enumType) {
            if (schemas.some(({ name }) => name === enumType.schema)) {
                return `Database[${JSON.stringify(enumType.schema)}]['Enums'][${JSON.stringify(enumType.name)}]`;
            }
        ...
        if (compositeType) {
            if (schemas.some(({ name }) => name === compositeType.schema)) {
                return `Database[${JSON.stringify(compositeType.schema)}]['CompositeTypes'][${JSON.stringify(compositeType.name)}]`;
            }
            return 'unknown';
        }
        return 'unknown';
