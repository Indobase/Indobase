import type { AuthChangeEvent } from '@indobaseinc/auth-js'
import { FunctionsClient } from '@indobaseinc/functions-js'
import {
  PostgrestClient,
...
} from '@indobaseinc/realtime-js'
import { StorageClient as SupabaseStorageClient } from '@indobaseinc/storage-js'
import {
...
import { applySettingDefaults, validateProjectUrl } from './lib/helpers'
...
> {
  ...
  constructor(
    ...
    if (!settings.accessToken) {
      ...
      this.auth = new Proxy<SupabaseAuthClient>({} as any, {
        get: (_, prop) => {
          throw new Error(
            `indobase-js: Supabase Client is configured with the accessToken option, accessing supabase.auth.${String(
              prop
            )} is not possible`
          )
        },
  ...
  private async _getAccessToken() {
    ...
    return data.session?.access_token ?? this.apiKey
  }
