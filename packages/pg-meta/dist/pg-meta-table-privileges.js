...
function retrieve({ id, name, schema = 'public', }) {
    if (id) {
        const sql = /* SQL */ `
with table_privileges as (${TABLE_PRIVILEGES_SQL})
select *
...
begin
${grants
        .map(({ privilegeType, relationId, grantee, isGrantable }) => `execute format('grant ${privilegeType} on table %s to ${grantee.toLowerCase() === 'public' ? 'public' : ident(grantee)} ${isGrantable ? 'with grant option' : ''}', ${relationId}::regclass);`)
        .join('\n')}
end $$;
`;
    return { sql };
...
begin
${revokes
        .map(({ privilegeType, relationId, grantee }) => `execute format('revoke ${privilegeType} on table %s from ${grantee.toLowerCase() === 'public' ? 'public' : ident(grantee)}', ${relationId}::regclass);`)
        .join('\n')}
end $$;
`;
    return { sql };
