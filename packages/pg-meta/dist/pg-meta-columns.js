...
function list({ tableId, includeSystemSchemas = false, includedSchemas, excludedSchemas, limit, offset, } = {}) {
    let sql = `
with
...
function create({ schema, table, name, type, default_value, default_value_format = 'literal', is_identity = false, identity_generation = 'BY DEFAULT', is_nullable, is_primary_key = false, is_unique = false, comment, check, }) {
...
function update(old, { name, type, drop_default = false, default_value, default_value_format = 'literal', is_identity, identity_generation = 'BY DEFAULT', is_nullable, is_unique, comment, check, }) {
    ...
    const typeSql = type === undefined
        ? ''
        : `ALTER TABLE ${ident(old.schema)}.${ident(old.table)} ALTER COLUMN ${ident(old.name)} SET DATA TYPE ${typeIdent(type)} USING ${ident(old.name)}::${typeIdent(type)};`;
    let defaultValueSql;
    if (drop_default) {
...
`;
    ...
    const checkSql = check === undefined
        ? ''
        : `
DO $$
DECLARE
...
function remove(column, { cascade = false } = {}) {
