...
export const getUsersCountSQL = ({ filter, keywords, providers, forceExactCount = false, column, scopedUserId, }) => {
    ...
    if (optimizedSearchMode) {
        if (column === 'email') {
            ...
            conditions.push(`lower(email) >= ${lowerBound}${upperBound ? ` and lower(email) < ${upperBound}` : ''} and instance_id = '00000000-0000-0000-0000-000000000000'::uuid`);
        }
        ...
        if (hasValidKeywords) {
            ...
            conditions.push(`id::text ilike ${escapedFilterKeywords} or email ilike ${escapedFilterKeywords} or phone ilike ${escapedFilterKeywords}`);
        }
        if (filter === 'verified') {
        ...
        if (providers && providers.length > 0) {
            ...
            if (providers.includes('saml 2.0')) {
                ...
                conditions.push(`(select jsonb_agg(case when value ~ '^sso' then 'sso' else value end) from jsonb_array_elements_text((raw_app_meta_data ->> 'providers')::jsonb)) ?| array[${literal(mappedProviders)}]`.trim());
            }
            else {
