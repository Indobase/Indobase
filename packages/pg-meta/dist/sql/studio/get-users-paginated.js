...
export const getPaginatedUsersSQL = ({ page = 0, verified, keywords, providers, sort, order, limit = DEFAULT_LIMIT, column, startAt, cursor, scopedUserId, improvedSearchEnabled = false, }) => {
    ...
    if (providers && providers.length > 0) {
        ...
        if (providers.includes('saml 2.0')) {
            conditions.push(`(select jsonb_agg(case when value ~ '^sso' then 'sso' else value end) from jsonb_array_elements_text((raw_app_meta_data ->> 'providers')::jsonb)) ?| array[${providers.map((p) => literal(p === 'saml 2.0' ? 'sso' : p)).join(', ')}]`.trim());
        }
        else {
            conditions.push(`(raw_app_meta_data->>'providers')::jsonb ?| array[${providers.map((p) => literal(p)).join(', ')}]`);
        }
    }
    if (scopedUserId) {
 ...
 */
export const getImprovedPaginatedUsersSQL = ({ column, keywords, verified, providers, sort, order, cursor, limit = DEFAULT_LIMIT, scopedUserId, }) => {
    const hasValidKeywords = keywords && keywords !== '';
    const conditions = [];
    ...
    if (hasValidKeywords) {
        if (column === 'email') {
            ...
            const range = stringRange(keywords);
            if (range[1]) {
                conditions.push(`raw_user_meta_data->>'name' >= ${literal(range[0])} AND raw_user_meta_data->>'name' < ${literal(range[1])}`);
            }
            else {
    ...
    if (providers && providers.length > 0) {
        if (providers.includes('saml 2.0')) {
            conditions.push(`(SELECT jsonb_agg(CASE WHEN value ~ '^sso' THEN 'sso' ELSE value END) FROM jsonb_array_elements_text((raw_app_meta_data ->> 'providers')::jsonb)) ?| array[${providers.map((p) => literal(p === 'saml 2.0' ? 'sso' : p)).join(', ')}]`);
        }
        else {
            conditions.push(`(raw_app_meta_data->>'providers')::jsonb ?| array[${providers.map((p) => literal(p)).join(', ')}]`);
        }
    ...
    if (cursor) {
        ...
        if (sortOn === 'id') {
            ...
            conditions.push(`(${ident(sortOn)}, id) ${operator} (${literal(cursor.sort)}, ${literal(cursor.id)}::uuid)`);
        }
    }
    const combinedConditions = conditions.map((x) => `(${x})`).join(' AND ');
