...
export function create({ name, schema = 'public', table, function_schema = 'public', function_name, function_args = [], activation, events, orientation, condition, }) {
    ...
    const sql = `create trigger ${ident(name)} ${activation} ${triggerEvents} on ${qualifiedTableName} ${triggerOrientation} ${triggerCondition} execute function ${qualifiedFunctionName}(${functionArgsStr});`;
    return {
        sql,
        zod: z.void(),
    };
...
export function update(id, params) {
    ...
    const updateNameSql = params.name && params.name !== id.name
