...
export const getDefaultOrderByColumns = (table, { excludedColumns = [] } = {}) => {
    ...
    if (table.columns && table.columns.length > 0) {
        const eligibleColumnsForSorting = table.columns.filter((x) => !x.data_type.includes('json') && !excludedColumns.includes(x.name));
        if (eligibleColumnsForSorting.length > 0)
            return [eligibleColumnsForSorting[0].name];
        else
            return [];
    }
    return [];
 ...
 */
export const shouldTruncateColumn = (columnFormat) => LARGE_COLUMNS_TYPES_SET.has(columnFormat.toLowerCase());
export const DEFAULT_PAGE_SIZE = 100;
export function getPagination(page, size = DEFAULT_PAGE_SIZE) {
    const limit = size;
    const from = page ? page * limit : 0;
    const to = page ? from + size - 1 : size - 1;
    return { from, to };
}
export const getTableRowsSql = ({ table, filters = [], sorts = [], page, limit, maxCharacters = MAX_CHARACTERS, maxArraySize = MAX_ARRAY_SIZE, sortExcludedColumns = [], }) => {
    ...
    filters.forEach((x) => {
        ...
        queryChains = queryChains.filter(x.column, x.operator, !isStringTypeColumn && x.value === '' ? null : x.value);
    });
    ...
    arrayBasedColumns.forEach(({ name: columnName, format }) => {
        ...
        const index = selectExpressions.findIndex((expr) => expr === ident(columnName) // if the column is selected without any truncation applied to it
        );
        // If the column is a json, the final cast remain an array of json
        const typeCast = JSON_SET.has(format) ? `${format}[]` : 'text[]';
        const lastElement = typeCast === 'text[]' ? `array['...']` : `array['{"truncated": true}'::json]`;
        if (index >= 0) {
