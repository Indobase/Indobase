...
export function countQuery(table, options) {
...
export function truncateQuery(table, options) {
...
export function deleteQuery(table, filters, options) {
...
export function insertQuery(table, values, options) {
    ...
    if (queryColumns.length == 0) {
        query = format('insert into %1$s select from jsonb_populate_recordset(null::%1$s, %2$s)', queryTable(table), literal(JSON.stringify(values)));
    }
    else {
        query = format('insert into %1$s (%2$s) select %2$s from jsonb_populate_recordset(null::%1$s, %3$s)', queryTable(table), queryColumns, literal(JSON.stringify(values)));
    }
    if (returning) {
        query +=
            enumArrayColumns === undefined || enumArrayColumns.length === 0
...
export function selectQuery(table, columns, options, isFinal = true, isCTE = false) {
    let query = '';
...
export function updateQuery(table, value, options) {
    ...
    const queryColumns = Object.keys(value)
        .map((x) => ident(x))
        .join(',');
    let query = format('update %1$s set (%2$s) = (select %2$s from json_populate_record(null::%1$s, %3$s))', queryTable(table), queryColumns, literal(JSON.stringify(value)));
    if (filters) {
        query = applyFilters(query, filters);
    }
    if (returning) {
...
function defaultTupleFilterSql(filter) {
    if (!Array.isArray(filter.column)) {
...
function inTupleFilterSql(filter) {
    if (!Array.isArray(filter.column)) {
...
function castColumnToText(filter) {
