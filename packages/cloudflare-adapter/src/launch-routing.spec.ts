...
describe('launch routing (hard Go Live path)', () => {
  ...
  it('hard rules forbid inventing live URLs and Connect language', () => {
    ...
    expect(LAUNCH_AGENT_HARD_RULES).toMatch(/same production job|executeProductionLaunchJob/i)
    ...
    expect(LAUNCH_AGENT_HARD_RULES).not.toMatch(/launchBusiness is preview\/draft only/)
    expect(LAUNCH_AGENT_HARD_RULES).not.toMatch(/Vercel|Netlify|GitHub Pages/i)
    expect(LAUNCH_SESSION_HINT).toMatch(/same production job|executeProductionLaunchJob/)
    expect(LAUNCH_SESSION_HINT).toMatch(/HARD PATH/)
    expect(LAUNCH_SESSION_HINT).not.toMatch(/launchBusiness is preview\/draft only/)
  })
})
