/**
 * Studio SSO — verifies handoff JWT from Studio (`aud=indobase-discuss`, HS256).
 * Same contract as `apps/studio/lib/api/saas/product-handoff.ts`.
 */
import { createHmac, randomBytes, timingSafeEqual } from 'node:crypto';
export const AUDIENCE = 'indobase-discuss';
export const SESSION_COOKIE = 'indobase_discuss_session';
export const SESSION_TTL_SECONDS = 60 * 60 * 12;
const POST_ROLES = new Set(['owner', 'admin', 'developer']);
...
export function resolveHandoffSecret() {
    const secret = (process.env.DISCUSS_HANDOFF_SECRET ||
        process.env.STUDIO_HANDOFF_SECRET ||
        '').trim();
    if (secret.length < 32) {
        throw new Error('DISCUSS_HANDOFF_SECRET missing or shorter than 32 chars');
    }
    return secret;
}
...
export function verifyStudioHandoff(token, secret) {
    ...
    return {
        ...
        organization_name: typeof payload.organization_name === 'string' ? payload.organization_name : undefined,
...
export function readSessionToken(value, secret) {
    ...
    try {
        ...
        if (typeof p.exp !== 'number' || p.exp < Math.floor(Date.now() / 1000))
            return null;
        const studioUrl = (typeof p.studio_url === 'string' && p.studio_url) ||
