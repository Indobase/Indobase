...
import { AUDIENCE, clearSessionCookie, createSessionToken, readCookie, readSessionToken, resolveHandoffSecret, sessionCookie, verifyStudioHandoff, } from './auth.js';
...
function isBridgeOwnedPath(pathname) {
...
function isNativeAuthPath(pathname) {
...
function forwardUpstreamCookies(upstream, c) {
...
// Brand assets
app.use('/brand/*', serveStatic({
    root: PUBLIC_ROOT,
}));
// AGPL notices (network users must reach without signing in)
app.get('/notices', (c) => c.html(`<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Open source notices — Indobase Discuss</title>
...
// ── SSO ──────────────────────────────────────────────────────────────────────
app.get('/sso/launch', (c) => c.html(`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Opening Indobase Discuss…</title>
...
app.get('/sso/health', async (c) => {
    ...
    ;
    body.upstreamReady = await pingUpstream();
    return c.json(body);
});
// ── Auth middleware ──────────────────────────────────────────────────────────
...
// ── Gameplan proxy ───────────────────────────────────────────────────────────
async function proxyGameplan(c) {
    if (!GAMEPLAN_UPSTREAM)
        return c.notFound();
...
async function proxyGameplanAuthenticated(c) {
