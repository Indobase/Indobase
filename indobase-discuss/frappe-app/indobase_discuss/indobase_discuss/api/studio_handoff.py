...
def _ensure_team(team_key: str, title: str, org_slug: str) -> str:
	...
	# GP Team uses Prompt naming — set name = team_key so /g/{team}/… deep links stay stable.
	doc = frappe.get_doc(
		{
			"doctype": "GP Team",
			"name": team_key,
...
def exchange(token: str | None = None) -> dict[str, str]:
	...
	if not token:
		# Frappe may pass JSON body as form; also accept request JSON.
		if frappe.request and frappe.request.is_json:
			token = (frappe.request.get_json(silent=True) or {}).get("token")
		if not token:
