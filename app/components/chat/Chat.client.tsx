...
export const ChatImpl = memo(
  ({ description, initialMessages, storeMessageHistory, importChat, exportChat }: ChatProps) => {
    ...
    const visibleProviders = useMemo(
      () =>
        activeProviders
          .filter((activeProvider) =>
            ALLOWED_CHAT_PROVIDER_NAMES.includes(activeProvider.name as (typeof ALLOWED_CHAT_PROVIDER_NAMES)[number]),
          )
          .sort(
            (left, right) =>
              ALLOWED_CHAT_PROVIDER_NAMES.indexOf(left.name as (typeof ALLOWED_CHAT_PROVIDER_NAMES)[number]) -
    ...
    const [provider, setProvider] = useState(() => {
      ...
      const savedAllowedProvider = visibleProviders.find((p) => p.name === savedProvider);

      return (savedAllowedProvider ||
        ALLOWED_CHAT_PROVIDERS.find((p) => p.name === savedProvider) ||
        DEFAULT_CHAT_PROVIDER) as ProviderInfo;
    });
    ...
    useEffect(() => {
      ...
      if (!visibleProviders.some((visibleProvider) => visibleProvider.name === provider.name)) {
        setProvider(visibleProviders[0]);
        Cookies.set('selectedProvider', visibleProviders[0].name, { expires: 30 });
      }
    }, [visibleProviders, provider.name]);
