  ...
  const filteredProviders = useMemo(() => {
    ...
    return providerList
      .map((provider) => {
        ...
        const match = fuzzyMatch(debouncedProviderSearchQuery, `${displayName} ${provider.name}`);

        return {
          ...provider,
          searchScore: match.score,
          searchMatches: match.matches,
          highlightedName: highlightText(displayName, debouncedProviderSearchQuery),
        };
      })
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 truncate">
              ...
              {provider?.name ? getProviderDisplayName(provider.name) : 'Select provider'}
            </div>
            <div
              className={classNames(
                ...
                <div className="px-3 py-3 text-sm">
                  ...
                  {debouncedProviderSearchQuery && (
                    <div className="text-xs text-bolt-elements-textTertiary">
                      Try searching for provider names like "OpenAI", "Claude", "OpenRouter", "Google", or "Deepseek"
                    </div>
                  )}
                </div>
              ) : (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        dangerouslySetInnerHTML={{
                          __html:
                            (providerOption as any).highlightedName || getProviderDisplayName(providerOption.name),
                        }}
                      />
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={classNames(
                      ...
                      'hover:bg-bolt-elements-background-depth-3',
                      showFreeModelsOnly
                        ? 'bg-accent-500/15 text-accent-800 dark:text-accent-300 border border-accent-500/30'
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1 ml-2">
                        {isModelLikelyFree(modelOption, provider?.name) && (
                          <span className="i-ph:gift text-xs text-accent-500" title="Free model" />
                        )}
