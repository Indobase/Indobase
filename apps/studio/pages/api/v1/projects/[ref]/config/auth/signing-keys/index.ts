...
async function handler(req: NextApiRequest, res: NextApiResponse, claims?: JwtPayload) {
  ...
  switch (req.method) {
    case 'GET': {
      try {
        const list = await listProjectSigningKeys({ claims: claims as any, ref })
    ...
    case 'POST': {
      ...
      try {
        const created = await createProjectSigningKey({
          claims: claims as any,
          ref,
          body: {
            algorithm: body?.algorithm,
            status: body?.status,
            private_jwk: body?.private_jwk,
          },
        })
