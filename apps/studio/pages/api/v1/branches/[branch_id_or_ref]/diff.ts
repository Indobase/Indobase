...
import { getBranchSchemaDiff, proxyBranchPlatformRequest } from 'lib/api/saas/branches'
...
async function handler(req: NextApiRequest, res: NextApiResponse, claims?: JwtPayload) {
  ...
  const proxied = await proxyBranchPlatformRequest(req, branchIdOrRef, '/diff')
  if (proxied) {
  ...
  try {
    ...
    const diff = await getBranchSchemaDiff({
      claims,
    ...
    console.error('[branch diff]', message, e)
    res.setHeader('Content-Type', 'text/plain; charset=utf-8')
