...
import { updateOsWorkspaceNames } from 'lib/api/saas/os-workspace-mutate'
...
/** POST — OS agent updateWorkspace (rename org / project). */
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const result = await updateOsWorkspaceNames({
      claims,
      workspaceRef,
      name: str(payload, 'name'),
      brand: str(payload, 'brand'),
      organizationName: str(payload, 'organization_name', 'organizationName', 'org_name'),
      projectName: str(payload, 'project_name', 'projectName', 'workspace_name'),
    })
    if (!result.ok) {
