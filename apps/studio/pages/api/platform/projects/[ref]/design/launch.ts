...
const designLaunchHandler = (req: NextApiRequest, res: NextApiResponse) =>
  apiWrapper(req, res, handler, { withAuth: true })

export default designLaunchHandler
...
async function handler(req: NextApiRequest, res: NextApiResponse, claims?: JwtPayload) {
  ...
  try {
    const response = await getDesignLaunchRedirect({
      claims,
      ref: ref.trim(),
    })
    ...
    const message = error instanceof Error ? error.message : 'Failed to launch Design'
