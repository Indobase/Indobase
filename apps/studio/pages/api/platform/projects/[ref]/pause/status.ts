...
import { getProjectPauseStatus } from 'lib/api/saas/project-lifecycle'
...
async function handler(req: NextApiRequest, res: NextApiResponse, claims?: JwtPayload) {
  ...
  try {
    const status = await getProjectPauseStatus({ claims, ref })
    return res.status(200).json(status)
  } catch (e) {
    const message = e instanceof Error ? e.message : 'Failed to load pause status'
