...
import { getOrganizationDailyUsage } from 'lib/api/saas/org-daily-usage'
...
async function handler(req: NextApiRequest, res: NextApiResponse, claims?: JwtPayload) {
  ...
  try {
    const usage = await getOrganizationDailyUsage({
      claims: claims ?? {},
      slug,
    ...
    const message = error instanceof Error ? error.message : 'Failed to load organization daily usage'
