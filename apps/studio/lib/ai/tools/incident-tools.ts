 ...
 *
 * @param baseUrl - The base URL for API requests (e.g., https://studio.indobase.in)
    ...
    execute: async () => {
      try {
        ...
        return {
          ...
          message: `There ${incidents.length === 1 ? 'is' : 'are'} ${incidents.length} active incident${incidents.length === 1 ? '' : 's'} on Indobase infrastructure. If the user's issue appears related, inform them about the ongoing incident(s) and direct them to https://status.indobase.in for real-time updates.`,
        }
      } catch (error) {
