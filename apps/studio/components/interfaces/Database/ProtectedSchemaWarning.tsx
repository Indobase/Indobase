...
export const ProtectedSchemaDialog = ({ onClose }: { onClose: () => void }) => {
  return (
    <>
      <DialogHeader>
        <DialogTitle>Schemas managed by Indobase</DialogTitle>
      </DialogHeader>
      <DialogSectionSeparator />
      <DialogSection className="space-y-2 prose">
        <p className="text-sm">
          The following schemas are managed by Indobase and are currently protected from write
        ...
        <p className="text-sm !mt-4">
          These schemas are critical to the functionality of your Indobase project and hence we
    ...
    <Admonition
      ...
      description={
        reason === 'fdw' && fdwType === 'iceberg' ? (
          <p>
            The <code className="text-code-inline">{schema}</code> schema is used by Indobase to
          ...
          <p>
            The <code className="text-code-inline">{schema}</code> schema is managed by Indobase and
