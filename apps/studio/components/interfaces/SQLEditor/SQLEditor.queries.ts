...
`.trim(),
  },
  {
    ...
    sql: `
--
-- For use with https://github.com/Indobase/supabase/tree/master/examples/slack-clone/nextjs-slack-clone
...
comment on column public.users.id is 'References the internal Indobase Auth user.';
...
--
-- For use with:
-- https://github.com/Indobase/supabase/tree/master/examples/todo-list/sveltejs-todo-list or
-- https://github.com/Indobase/examples-archive/tree/mainindobase-js-v1/todo-list
--
...
/**
* This trigger automatically creates a user entry when a new user signs up via Indobase Auth.
*/
create function public.handle_new_user()
returns trigger
set search_path = ''
...
`.trim(),
  },
  {
    ...
    description: 'Sets up a the Schema and Tables for the NextAuth Indobase Adapter.',
    sql: `
--
...
from http(
    (
        ...
        || '&limit=1',
        array[
            ('apiKey', '<YOUR_DATABASE_DEV_API_KEY>')::http_header
        ],
        null,
        null
    )
