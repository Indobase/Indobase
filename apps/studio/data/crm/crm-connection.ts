...
import type { CrmClientVariables } from './crm-client'

/**
 * Everything the CRM client needs, assembled from the queries Studio already has.
 *
 * CRM is a Studio surface, not a separate app: there is no handoff, no second login and no
 ...
 */
export function useCrmConnection({ projectRef }: { projectRef?: string }) {
  ...
  const connection: CrmClientVariables = {
    projectRef,
