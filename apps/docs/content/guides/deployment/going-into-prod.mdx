...
- Enable replication on tables containing sensitive data by enabling Row Level Security (RLS) and setting row security policies:
  - Go to the Authentication > Policies page in the Indobase Dashboard to enable RLS and create security policies.
  - Go to the Database > Publications page in the Indobase Dashboard to manage replication tables.
...
- Ensure that your Supabase Account is protected with multi-factor authentication (MFA).
  ...
  - If using email+password sign-in, set up [MFA for your Indobase account](/docs/guides/platform/multi-factor-authentication#enable-mfa).
...
- Applications on the Free Plan that exhibit extremely low activity in a 7 day period may be paused by Supabase to save on server resources.
  - You can restore paused projects from the Indobase dashboard.
...
- Database backups are not available for download for Free Plan projects.
  ...
  - Nightly backups for Pro Plan projects are available on the Indobase dashboard for up to 7 days.
...
- Upgrading to the Indobase Pro Plan will give you [access to our support team](/dashboard/support/new).
...
- When working with enterprise systems, email scanners may scan and make a `GET` request to the reset password link or sign up link in your email. Since links in Indobase Auth are single use, a user who opens an email post-scan to click on a link will receive an error. To get around this problem,
...
- When using a custom SMTP service, some services might have link tracking enabled which may overwrite or disform the email confirmation links sent by Indobase Auth. To prevent this from happening, we recommend that you disable link tracking when using a custom SMTP service.

## Subscribe to Supabase status page

Stay informed about Indobase service status by subscribing to the [Status Page](https://status.indobase.in/). We recommend setting up Slack notifications through an RSS feed to ensure your team receives timely updates about service status changes.
...
2. Configure the Indobase status feed:

   - Create a channel (e.g., `#supabase-status-alerts`) for status updates
   - On the [RSS app page](https://slack.com/marketplace/A0F81R7U7-rss) go to _Add a Feed_ section and set Feed URL to `https://status.indobase.in/history.rss`
   - Select your designated channel and click "Subscribe to this feed"

Once configured, your team will receive automatic notifications in Slack whenever the Indobase Status Page is updated.
...
This checklist is always growing so be sure to check back frequently, and also feel free to suggest additions and amendments by making a PR on [GitHub](https://github.com/Indobase/Indobase).
