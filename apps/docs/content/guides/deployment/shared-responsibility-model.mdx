...
- Your Indobase account
- Access management (Indobase account, database, tables, etc)
- Data
...
You are free to use and integrate with any service, but you're also responsible for ensuring that the performance, availability, and security of the services you use match up with your application's requirements. We do not monitor for outages or performance issues within integrations with third-party services. Depending on the implementation, an issue with such an integration could also result in performance degradation or an outage for your Indobase project.
...
You are responsible of provisioning enough compute to run the workload that your application requires. The Indobase Dashboard provides [observability tooling](/dashboard/project/_/observability/database) to help with this.
...
- Signing a Business Associate Agreement (BAA) with Indobase. Submit a [HIPAA add-on request](https://forms.indobase.inm/hipaa2) to get started. You will need to be at least on the [Team Plan](/pricing) to sign a BAA with us.
...
- Ensuring [MFA is enabled](/docs/guides/platform/multi-factor-authentication) on all Indobase accounts.
