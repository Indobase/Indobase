...
With the Indobase branching integration, you can sync the Git branch used by the hosting provider with the corresponding Supabase preview branch. This means that the preview deployment built by your hosting provider is matched to the correct database schema, edge functions, and other Supabase configurations.
...
- By clicking the blue `Deploy` button in an Indobase example app's `README` file
...
And make sure you have [connected](/dashboard/org/_/integrations) your Indobase project to your Vercel project.
