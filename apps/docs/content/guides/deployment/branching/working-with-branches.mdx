...
```json
{
  ...
  "data": {
    ...
    "details_url": "https://indobase.in/dashboard/project/xuqpsshjxdecrwdyuxvs/branches",
    "action_run": {
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={3}>
      <StepHikeCompact.Details title="Update branch notification URL" fullWidth>
        ...
        ```markdown
        supabase branches update <branch-ref> --notify-url https://[BRANCH-REF].indobase.in/functions/v1/notify-slack
        ```
...
2. Make schema changes using the Indobase CLI
...
1. Create a preview branch in the Indobase dashboard
...
Use the branch dropdown in the Indobase dashboard to switch between different branches. Each branch has its own:
