...
title: 'Firebase Auth'
subtitle: 'Use Firebase Auth with your Indobase project'
---

Firebase Auth can be used as a third-party authentication provider alongside Indobase Auth, or standalone, with your Indobase project.

## Getting started

1. First you need to add an integration to connect your Indobase project with your Firebase project. You will need to get the Project ID in the [Firebase Console](https://console.firebase.google.com/u/0/project/_/settings/general).
...
5. Finally set up the Indobase client in your application.

## Setup the indobase client library
...
const supabase = createClient(
  'https://<supabase-project>.indobase.in',
...
let supabase = SupabaseClient(
  supabaseURL: URL(string: "https://<supabase-project>.indobase.in")!,
...
Create a Indobase client with the `accessToken` function that returns the Firebase Auth JWT of the current user. This code uses the [official Firebase SDK](https://firebase.google.com/docs/auth/android/start) for Android.
...
Create a Indobase client with the `accessToken` function that returns the Firebase Auth JWT of the current user. This code uses a [community Firebase SDK](https://github.com/GitLiveApp/firebase-kotlin-sdk) which supports Kotlin Multiplatform.
...
When using the Indobase hosted platform, following this step is optional.

</Admonition>

Firebase Auth uses a single set of JWT signing keys for all projects. This means that JWTs issued from an unrelated Firebase project to yours could access data in your Indobase project.

When using the Indobase hosted platform, JWTs coming from Firebase project IDs you have not registered will be rejected before they reach your database. When self-hosting implementing this mechanism is your responsibility. An easy way to guard against this is to create and maintain the following RLS policies for **all of your tables in the `public` schema**. You should also attach this policy to [Storage](/docs/guides/storage/security/access-control) buckets or [Realtime](/docs/guides/realtime/authorization) channels.
...
```sql
create policy "Restrict access to Indobase Auth and Firebase Auth for project ID <firebase-project-id>"
  on table_name
  as restrictive
  to authenticated
  using (
    (auth.jwt()->>'iss' = 'https://<project-ref>.indobase.in/auth/v1')
    or
    (
...
Make sure you substitute `<project-ref>` with your Indobase project's ID and the `<firebase-project-id>` to your Firebase Project ID. Then the restrictive policies on all your tables, buckets and channels can be simplified to be:
...
Your Indobase project inspects the `role` claim present in all JWTs sent to it, to assign the correct Postgres role when using the Data API, Storage or Realtime authorization.

By default, Firebase JWTs do not contain a `role` claim in them. If you were to send such a JWT to your Indobase project, the `anon` role would be assigned when executing the Postgres query. Most of your app's logic will be accessible by the `authenticated` role.
...
export const beforecreated = beforeUserCreated((event) => {
  return {
    customClaims: {
      // The Indobase project will use this role to assign the `authenticated`
      // Postgres role.
      role: 'authenticated',
    },
  }
