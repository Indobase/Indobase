...
title: 'Clerk'
subtitle: 'Use Clerk with your Indobase project'
---

Clerk can be used as a third-party authentication provider alongside Indobase Auth, or standalone, with your Indobase project.
...
Finally add a [new Third-Party Auth integration with Clerk](/dashboard/project/_/auth/third-party) in the Indobase dashboard.

### Configure for local development or self-hosting

When developing locally or self-hosting with the Indobase CLI, add the following config to your `supabase/config.toml` file:
...
2. Once all Clerk session tokens for your instance contain the `role` claim, add a [new Third-Party Auth integration with Clerk](/dashboard/project/_/auth/third-party) in the Indobase dashboard or register it in the CLI as instructed above.
...
Once you've configured the Indobase client library to use Clerk session tokens, you can use RLS policies to secure access to your project's database, Storage objects and Realtime channels.
