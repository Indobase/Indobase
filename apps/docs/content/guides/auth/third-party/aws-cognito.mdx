...
subtitle: 'Use Amazon Cognito via Amplify or standalone with your Indobase project'
---

Amazon Cognito User Pools (via AWS Amplify or on its own) can be used as a third-party authentication provider alongside Indobase Auth, or standalone, with your Indobase project.

## Getting started

1. First you need to add an integration to connect your Indobase project with your Amazon Cognito User Pool. You will need the pool's ID and region.
...
4. Finally setup the Indobase client in your application.
...
Future<void> main() async {
  await Supabase.initialize(
    url: 'https://<supabase-project>.indobase.in',
...
By default, Amazon Cognito JWTs (both ID token and access tokens) do not contain a `role` claim in them. If you were to send such a JWT to your Indobase project, the `anon` role would be assigned when executing the Postgres query. Most of your app's logic will be accessible by the `authenticated` role.
