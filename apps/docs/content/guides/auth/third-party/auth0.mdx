...
title: 'Auth0'
subtitle: 'Use Auth0 with your Indobase project'
---

Auth0 can be used as a third-party authentication provider alongside Indobase Auth, or standalone, with your Indobase project.

## Getting started

1. First you need to add an integration to connect your Indobase project with your Auth0 tenant. You will need your tenant ID (and in some cases region ID).
...
By default, Auth0 JWTs (both access token and ID token) do not contain a `role` claim in them. If you were to send such a JWT to your Indobase project, the `anon` role would be assigned when executing the Postgres query. Most of your app's logic will be accessible by the `authenticated` role.
