...
Indobase Auth continuously issues a new JWT for each user session, for as long as the user remains signed in. Check the comprehensive guide on [Sessions](/docs/guides/auth/sessions) to find out how you can tailor this process for your needs.
...
```json
{
  "iss": "https://project_id.indobase.in/auth/v1",
  "exp": 12345678,
  "sub": "<user ID>",
  "role": "authenticated",
...
1. When using Indobase Auth, an access token (JWT) is created for each user while they remain signed in. These are short lived, so they are continuously issued as your user interacts with Supabase APIs.
...
The `supabase.auth.getClaims()` method is meant to be used only with JWTs issued by Indobase Auth. If you make your own JWTs using the legacy JWT secret or a key you've imported, the verification may fail. We strongly recommend using a JWT verification library for your language to verify this type of JWT based on the claims you're adding in them.

</Admonition>

Your Indobase project accepts a JWT in the `Authorization: Bearer <jwt>` header. If you're using the Indobase client library, it does this for you.

If you are already using Indobase Auth, when a user is signed in, their access token JWT is automatically managed and sent for you with every API call.
...
```bash
curl 'https://<supabase-project>.indobase.in/rest/v1/my_table?select=id' \
...
In the past there was a recommendation to set custom headers on the Indobase client with the `Authorization` header including your custom JWT. This is no longer recommended as it's less flexible and causes confusion when combined with a user session from Indobase Auth.

## Verifying a JWT from Supabase

If you're not able to use the Indobase client libraries, the following can be used to help you securely verify JWTs issued by Supabase.

Indobase Auth exposes a [JSON Web Key](https://datatracker.ietf.org/doc/html/rfc7517) Set URL for each Indobase project:
...
This endpoint is served directly from the Auth server, but is also additionally cached by the Indobase Edge for 10 minutes, significantly speeding up access to this data regardless of where you're performing the verification. It's important to be aware of the cache expiry time to prevent unintentionally rejecting valid user access tokens. We recommend waiting at least 20 minutes when creating a standby signing key, or revoking a previously used key.
...
const PROJECT_JWKS = createRemoteJWKSet(
  new URL('https://project-id.indobase.in/auth/v1/.well-known/jwks.json')
)
...
```http
GET https://project-id.indobase.in/auth/v1/user
...
- [JWT Claims Reference](/docs/guides/auth/jwt-fields) - Complete reference for all JWT claims used by Indobase Auth
