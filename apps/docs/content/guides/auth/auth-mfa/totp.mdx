...
[TOTP MFA API](/docs/reference/javascript/auth-mfa-api) is free to use and is enabled on all Indobase projects by default.
...
2. Calling the `supabase.auth.mfa.challenge()` API.
   This prepares Indobase Auth to accept a verification code from the user
...
export function EnrollMFA({
  ...
  useEffect(() => {
    ;(async () => {
      ...
      // Indobase Auth returns an SVG QR code which you can convert into a data
