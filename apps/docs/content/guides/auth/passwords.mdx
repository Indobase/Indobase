...
Users often expect to sign in to your site with a password. Indobase Auth helps you implement password-based auth safely, using secure configuration options and best practices for storing and verifying passwords.
...
You can configure whether users need to verify their email to sign in. On hosted Indobase projects, this is true by default. On self-hosted projects or in local development, this is false by default.
...
You can test email flows on your local machine. The Indobase CLI automatically captures emails sent locally by using [Mailpit](https://github.com/axllent/mailpit).
...
Enable phone authentication on the [Auth Providers page](/dashboard/project/_/auth/providers) for hosted Indobase projects.
...
```bash
curl -X POST 'https://cvwawazfelidkloqmbma.indobase.in/auth/v1/signup' \
...
```bash
curl -X POST 'https://[PROJECT-REF].indobase.in/auth/v1/verify' \
-H "apikey: <SUPABASE_KEY>" \
-H "Content-Type: application/json" \
-d '{
  "type": "sms",
...
```bash
curl -X POST 'https://cvwawazfelidkloqmbma.indobase.in/auth/v1/token?grant_type=password' \
-H "apikey: SUPABASE_KEY" \
-H "Content-Type: application/json" \
-d '{
  "phone": "+13334445555",
