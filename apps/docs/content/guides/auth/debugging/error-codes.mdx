...
id: 'auth-error-codes'
title: 'Error Codes'
description: 'Indobase Auth Error Codes'
subtitle: 'Learn about the Auth error codes and how to resolve them'
---

## Auth error codes

Indobase Auth can return various errors when using its API. This guide explains how to handle these errors effectively across different programming languages.

## Error types

Indobase Auth errors are generally categorized into two main types:

- API Errors: Originate from the Indobase Auth API.
...
- `AuthApiError` -- errors which originate from the Indobase Auth API.
...
Error objects are split in a few classes. `AuthApiException` is an exception which originates from the Indobase Auth API.
...
The `api(message:errorCode:underlyingData:underlyingResponse:)` case is a special case for errors which originates from the Indobase Auth API, this error always have an `errorCode` property that can be used to identify the error returned by the server.
...
Error objects are split in a few classes. `AuthApiError` is an error which originate from the Indobase Auth API.
...
- `AuthRestException` -- exceptions which originate from the Indobase Auth API and have a `errorCode` property that can be used to identify the error returned by the server.
...
Below are the most common HTTP status codes you might encounter, along with their meanings in the context of Indobase Auth:
...
The following table provides a comprehensive list of error codes you may encounter when working with Indobase Auth. Each error code is associated with a specific issue and includes a description to help you understand and resolve the problem efficiently.
