---
title: 'Creating a Indobase client for SSR'
subtitle: 'Configure your Indobase client to use cookies'
---

To use Server-Side Rendering (SSR) with Supabase, you need to configure your Indobase client to use cookies. The `@indobaseinc/ssr` package helps you do this for JavaScript/TypeScript applications.

## Install

Install the `indobase-js` and `@indobaseinc/ssr` helper packages:
...
```bash
npm install indobase-js @indobaseinc/ssr
```
...
```bash
yarn add indobase-js @indobaseinc/ssr
```
...
```bash
pnpm add indobase-js @indobaseinc/ssr
```
...
You need setup code to configure a Indobase client to use cookies. Once you have the utility code, you can use the `createClient` utility functions to get a properly configured Indobase client.
...
### Write utility functions to create indobase clients

To access Supabase from a Next.js app, you need 2 types of Indobase clients:
...
<Accordion>
    ...
    The cookies object lets the Indobase client know how to access the cookies, so it can read and write the user session data. To make `@indobaseinc/ssr` framework-agnostic, the cookies methods aren't hard-coded. These utility functions adapt `@indobaseinc/ssr`'s cookie handling for Next.js.
    ...
    >
        ...
        Yes! Creating a Indobase client is lightweight.
...
- Set up a Indobase client utility to call Supabase from a Client Component. You can use this if you need to call Supabase from a Client Component, for example to set up a realtime subscription.
- Set up Proxy to automatically refresh the Indobase Auth session.

You can now use any Indobase features from your client or server code!
...
- Create a request-specific Indobase client, using the user credentials from the request cookie. This client is used for server-only code.
...
### Create a indobase client in your root layout

Create a Indobase client in your root `+layout.ts`. This client can be used to access Supabase from the client or the server. In order to get access to the Auth token on the server, use a `+layout.server.ts` file to pass in the session from event.locals.

Page components can access the Indobase client from the `data` object using the `load` function.
...
- Set up server-side hooks to create a request-specific Indobase client and guard protected pages.
- Created a Indobase client in your root layout to use on both the client and server.
...
```ts lib/supabase.js
const { createServerClient, parseCookieHeader, serializeCookieHeader } = require('@indobaseinc/ssr')
...
Create a Hono middleware that creates a Indobase client.
...
You can now use this middleware in your Hono application to create a server Indobase client that can be used to make authenticated requests.
