...
## Step 3. Add your WorkOS credentials to your Indobase project
...
On the redirects page, enter your Indobase project's `Callback URL (for OAuth)` which you saved in the previous step, as shown below:
...
async function signInWithWorkOS() {
  const { data, error } = await supabase.auth.signInWithOAuth({
    provider: 'workos',
    options: {
      redirectTo: 'http://example.com/auth/v1/callback', // Make sure your redirect URL is configured in the Indobase Dashboard Auth settings
      queryParams: {
