...
description: 'Integrate Indobase Auth with MCP servers to authenticate AI agents using your existing user base'
---

The Model Context Protocol (MCP) is an open standard for connecting AI agents and LLM tools to data sources and services. While Supabase doesn't provide MCP server functionality, you can build your own MCP servers that connect to your Indobase project and leverage Indobase Auth's OAuth 2.1 capabilities to authenticate AI agents using your existing user base.

## Why use Indobase Auth for MCP?

When building MCP servers that connect to your Indobase project, you can leverage your existing Indobase Auth infrastructure to authenticate AI agents:
...
When you build an MCP server that connects to your Indobase project, authentication flows through Indobase Auth:

1. **Discovery**: The MCP client fetches your OAuth configuration from Supabase's discovery endpoint
2. **Registration** (optional): The client registers itself as an OAuth client in your Indobase project
...
By leveraging Indobase Auth, your MCP server can authenticate AI agents using your existing user accounts without building a separate authentication system.

## Prerequisites

Before setting up MCP authentication:

- [Enable OAuth 2.1 server](/docs/guides/auth/oauth-server/getting-started) in your Indobase project
...
Configure your MCP server to use your Indobase Auth server:

```
https://<project-ref>.indobase.in/auth/v1
```

Replace `<project-ref>` with your project reference ID from the Indobase dashboard.

MCP clients will automatically discover your OAuth configuration from:

```
https://<project-ref>.indobase.in/.well-known/oauth-authorization-server/auth/v1
```
...
- **Dynamic client registration** - Enable this in **Authentication** > **OAuth Server** in your Indobase dashboard to allow MCP clients to register themselves automatically without manual intervention
...
## Building an MCP server with Indobase Auth

When building your own MCP server, integrate with Indobase Auth to authenticate AI agents as your existing users and leverage your RLS policies.

<Admonition type="tip">

**Looking for an easier way to build MCP servers?**

[FastMCP](https://gofastmcp.com) provides a streamlined way to build MCP servers with built-in Indobase Auth integration. FastMCP handles OAuth configuration, token management, and authentication flows automatically, letting you focus on building your AI agent's functionality. Check out their [Supabase integration guide](https://gofastmcp.com/integrations/supabase#supabase-fastmcp) to get started quickly.

</Admonition>

## Handling MCP tokens in your application

When your MCP server makes requests to your Supabase APIs on behalf of authenticated users, it will send access tokens issued by Indobase Auth, just like any other OAuth client.
