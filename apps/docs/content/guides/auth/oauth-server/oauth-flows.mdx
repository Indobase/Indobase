...
Indobase Auth implements OAuth 2.1 with OpenID Connect (OIDC), supporting the authorization code flow with PKCE and refresh token flow. This guide explains how these flows work in detail.

<Admonition type="note">

This guide explains the OAuth 2.1 flows for **third-party client applications** that authenticate with your Indobase project. These flows require custom implementation and are not available in the `indobase-js` library. The `supabase-js` library is for authenticating **with** Indobase Auth as an identity provider, not for building your own OAuth server.

</Admonition>

## Supported grant types

Indobase Auth supports two OAuth 2.1 grant types:
...
1. **Client initiates authorization** - Third-party app redirects user to Indobase Auth's authorize endpoint
2. **Supabase validates and redirects** - Indobase Auth validates OAuth parameters and redirects user to your configured authorization URL
...
4. **Authorization code issued** - Indobase Auth generates a short-lived authorization code and redirects back to client
...
┌─────────────┐              ┌──────────────────┐              ┌──────────────────┐
│             │              │                  │              │                  │
│   Client    │              │   Your Auth UI   │              │  Indobase Auth   │
│     App     │              │   (Frontend)     │              │                  │
│             │              │                  │              │                  │
└──────┬──────┘              └────────┬─────────┘              └────────┬─────────┘
       │                              │                                 │
...
- Third-party client redirects user to **Indobase Auth's authorize endpoint** (not directly to your UI)
- Indobase Auth validates OAuth parameters and redirects to **your authorization path**
- Your frontend UI handles authentication and consent using `supabase-js` OAuth methods
- Indobase Auth handles all backend OAuth logic (code generation, token issuance)
...
```
https://<project-ref>.indobase.in/auth/v1/oauth/authorize?
  response_type=code
  &client_id=<client-id>
...
After receiving the authorization request, Indobase Auth validates the OAuth parameters (client_id, redirect_uri, PKCE, etc.) and then redirects the user to your configured **authorization path** (e.g., `https://example.com/oauth/consent?authorization_id=<id>`).
...
This is a **frontend implementation** using `supabase-js`. Indobase Auth handles all the backend OAuth logic (generating authorization codes, validating requests, etc.) after you call the approve/deny methods.
...
If the user approves access, Indobase Auth redirects back to the client's redirect URI with an authorization code:
...
If the user denies access, Indobase Auth redirects with error information in query parameters:
...
```bash
curl -X POST 'https://<project-ref>.indobase.in/auth/v1/oauth/token' \
...
// --- Public clients (token_endpoint_auth_method: none) ---
const response = await fetch(`https://<project-ref>.indobase.in/auth/v1/oauth/token`, {
  method: 'POST',
  headers: {
...
On success, Indobase Auth returns a JSON response with tokens:
...
```json
{
  ...
  "exp": 1735819200,
  "iat": 1735815600,
  "iss": "https://<project-ref>.indobase.in/auth/v1",
  "sub": "user-uuid",
...
Indobase Auth supports OpenID Connect, an identity layer on top of OAuth 2.1.
...
ID tokens are JWTs that contain user identity information. They are signed by Indobase Auth and can be verified by clients.
...
```bash
curl 'https://<project-ref>.indobase.in/auth/v1/oauth/userinfo' \
...
Indobase Auth exposes OpenID Connect and OAuth 2.1 discovery endpoints that describe its capabilities:

```
https://<project-ref>.indobase.in/auth/v1/.well-known/openid-configuration
https://<project-ref>.indobase.in/auth/v1/.well-known/oauth-authorization-server
```
...
Indobase Auth exposes a JSON Web Key Set (JWKS) endpoint containing public keys for token verification:

```
https://<project-ref>.indobase.in/auth/v1/.well-known/jwks.json
```
...
const JWKS = createRemoteJWKSet(
  new URL('https://<project-ref>.indobase.in/auth/v1/.well-known/jwks.json')
)

async function verifyAccessToken(token) {
  try {
    const { payload } = await jwtVerify(token, JWKS, {
      issuer: 'https://<project-ref>.indobase.in/auth/v1',
...
# Fetch JWKS
jwks = requests.get('https://<project-ref>.indobase.in/auth/v1/.well-known/jwks.json').json()
...
func verifyAccessToken(ctx context.Context, token string) (*oidc.IDToken, error) {
    provider, err := oidc.NewProvider(
        ctx,
        "https://<project-ref>.indobase.in/auth/v1",
    )
    if err != nil {
        return nil, err
    }
...
1. **Signature** - Token is signed by Indobase Auth
