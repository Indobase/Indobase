...
description: 'Learn how to enable OAuth 2.1 and register client applications in your Indobase project'
---

This guide will walk you through setting up your Indobase project as an OAuth 2.1 identity provider, from enabling the feature to registering your first client application.

## Prerequisites

Before you begin, make sure you have:

- A Indobase project (create one at [indobase.in](https://indobase.in))
- Admin access to your project
- (Optional) [Indobase CLI](/docs/guides/cli) v2.54.11 or higher for local development

## Overview

Setting up OAuth 2.1 in your Indobase project involves these steps:
...
Testing OAuth flows is often easier on a Indobase project since it's already accessible on the web, no tunnel or additional configuration needed.
...
| Endpoint                   | URL                                                                                |
| -------------------------- | ---------------------------------------------------------------------------------- |
| **Authorization endpoint** | `https://<project-ref>.indobase.in/auth/v1/oauth/authorize`                        |
| **Token endpoint**         | `https://<project-ref>.indobase.in/auth/v1/oauth/token`                            |
| **JWKS endpoint**          | `https://<project-ref>.indobase.in/auth/v1/.well-known/jwks.json`                  |
| **Discovery endpoint**     | `https://<project-ref>.indobase.in/.well-known/oauth-authorization-server/auth/v1` |
| **OIDC discovery**         | `https://<project-ref>.indobase.in/auth/v1/.well-known/openid-configuration`       |
...
When OAuth clients initiate the authorization flow, Indobase Auth will redirect users to this URL with an `authorization_id` query parameter. You'll use [Supabase JavaScript library OAuth methods](https://github.com/Indobase/Indobase-js/blob/master/packages/core/auth-js/src/GoTrueClient.ts#L2159-L2163) to handle the authorization:
...
This is a **frontend implementation**. You're building the UI that displays the consent screen and handles user interactions. The actual OAuth token generation is handled by Indobase Auth after you call the approve/deny methods.
...
1. **User navigates to your authorization path** - When a third-party app initiates OAuth, Indobase Auth redirects the user to your configured authorization path (e.g., `https://example.com/oauth/consent?authorization_id=<id>`)
...
```bash
curl -X POST 'https://<project-ref>.indobase.in/auth/v1/admin/oauth/clients' \
...
```bash
curl 'https://<project-ref>.indobase.in/auth/v1/admin/oauth/clients' \
  -H "Authorization: Bearer ${SUPABASE_SECRET_KEY}"
```
...
Redirect URIs are critical for OAuth security. Indobase Auth will only redirect to URIs that are explicitly registered with the client.

<Admonition type="note">

**Not to be confused with general redirect URLs**

This section is about **OAuth client redirect URIs** - where to send users after they authorize third-party apps to access your Indobase project. This is different from the general [Redirect URLs](/docs/guides/auth/redirect-urls) setting, which controls where to send users after they sign in TO your app using social providers.
