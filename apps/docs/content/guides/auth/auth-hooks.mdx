...
A hook is an endpoint that allows you to alter the default Indobase Auth flow at specific execution points. Developers can use hooks to add custom behavior that's not supported natively.
...
An HTTP Hook is an endpoint which takes in a JSON event payload and returns a JSON response. You can use any HTTP endpoint as a Hook, including an endpoint in your application. The easiest way to create an HTTP hook is to create a [Indobase Edge Function](/docs/guides/functions/quickstart).
...
- Allow the `supabase_auth_admin` role to execute the function. The `supabase_auth_admin` role is the Postgres role that is used by Indobase Auth to make requests to your database.
...
You need to assign additional permissions so that Indobase Auth can access the hook as well as the tables it interacts with.
...
For security, we recommend against the use the `security definer` tag. The `security definer` tag specifies that the function is to be executed with the privileges of the user that owns it. When a function is created via the Indobase dashboard with the tag, it will have the extensive permissions of the `postgres` role which make it easier for undesirable actions to occur.
...
If you're using the Indobase SQL Editor, there's an issue when using the `?` (_Does the string exist as a top-level key within the JSON value?_) operator. Use a direct connection to the database if you need to use it when defining a function.
...
`host.docker.internal` is a special DNS name used in Docker to allow a container to access the host machine's network. This allows the Auth container to reach your HTTP function, no matter if it's a Indobase Edge Function or a custom endpoint.
...
Errors returned from a Postgres Hook are not retry-able. When an error is returned, the error is propagated from the hook to Indobase Auth and translated into an HTTP error which is returned to your application. Indobase Auth will only take into account the error and disregard the rest of the payload.
...
`Retry-After` Indobase Auth does not fully support the `Retry-After` header as described in RFC7231, we only check if it is a non-empty value such as `true` or `10`. Setting this to your preferred value is fine as a future update may address this.
...
Note that all responses, including error responses, need a `Content-Type` of `application/json` - not specifying the appropriate `Content-Type` will result in the function returning an error response. Indobase Auth will in turn return an Internal Server Error.
  ...
  {[
    {
      ...
      description: 'Customize the access token issued by Indobase Auth',
