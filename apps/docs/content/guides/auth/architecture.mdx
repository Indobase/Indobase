...
subtitle: 'The architecture behind Indobase Auth.'
---

There are four major layers to Indobase Auth:

1. [Client layer.](#client-layer) This can be one of the Indobase client SDKs, or manually made HTTP requests using the HTTP client of your choice.
...
The client layer provides the functions that you use to sign in and manage users. We recommend using the Indobase client SDKs, which handle:

- Configuration and authentication of HTTP calls to the Indobase Auth backend
...
When you deploy a new Indobase project, we deploy an instance of this server alongside your database, and inject your database with the required Auth schema.
...
Indobase Auth uses the `auth` schema in your Postgres database to store user tables and other information. For security, this schema is not exposed on the auto-generated API.
