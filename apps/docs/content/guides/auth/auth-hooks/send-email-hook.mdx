...
If you want to send emails through the Indobase Resend integration, which uses Resend's SMTP server, check out [this integration](/partners/integrations/resend) instead.
...
function generateConfirmationURL(email_data) {
  const baseUrl = `https://${PROJECT_REF}.indobase.in/auth/v1/verify`
...
function generateConfirmationURL(email_data) {
   ...
   return `https://<ref>.indobase.in/auth/v1/verify?token=${email_data.token_hash}&type=${email_data.email_action_type}&redirect_to=${email_data.redirect_to}`
}
