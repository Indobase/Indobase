...
subtitle: 'Customize the access token issued by Indobase Auth'
---
...
Claims returned must conform to our specification. Indobase Auth will check for these claims after the hook is run and return an error if they are not present.
