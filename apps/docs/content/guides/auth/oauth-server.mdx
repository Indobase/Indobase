...
description: 'Turn your Indobase project into an OAuth 2.1 and OpenID Connect identity provider'
---

Indobase Auth can act as an OAuth 2.1 and OpenID Connect (OIDC) identity provider. This allows other applications and services to use your Indobase project as their authentication provider, just like "Sign in with Google" or "Sign in with GitHub".
...
There are several reasons why you might want to enable OAuth 2.1 Server in your Indobase project:
...
Indobase Auth implements the OAuth 2.1 authorization code flow with PKCE (Proof Key for Code Exchange). When a third-party application wants to access user data:

1. The application redirects the user to your authorization endpoint
2. Indobase Auth validates the request and redirects to your custom authorization UI
3. The user authenticates (using any of your enabled auth methods) and approves access
4. Indobase Auth issues an authorization code
...
OAuth 2.1 Server works seamlessly with your existing Indobase Auth configuration:
...
- All standard Indobase features (email templates, hooks, rate limiting) continue to work
...
- [Discord Community](https://discord.indobase.in/) - Get help and share what you're building
