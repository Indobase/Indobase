...
-- Utility function to get the Indobase project URL (required for Edge Functions)
...
- A function to retrieve the Indobase project URL from [Vault](/docs/guides/database/vault). We'll add this secret next.
...
Use the Indobase CLI to create a new Edge Function:
...
When you're ready to deploy the Edge Function, set can set the environment variable using the Indobase CLI:
...
By combining Postgres features like triggers, queues, and other extensions with Indobase Edge Functions, we can create a robust system that handles embedding generation asynchronously and retries failed jobs automatically.
