...
You can use the Indobase vector database functionality to store and query CLIP embeddings.
...
You can store your image embeddings in Supabase using the Indobase `vecs` Python package:
...
Replace `DB_CONNECTION` with the authentication information for your database. You can retrieve this from the Indobase dashboard in `Project Settings > Database Settings`.
