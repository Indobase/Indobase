...
```bash
npm install indobase-js fetch-retry
```
...
// Create a Indobase client instance with the custom fetch
...
## 4. Using the indobase client

With `fetch-retry` integrated, you can use the Indobase client as usual. The retry logic will automatically apply to all network requests made by `supabase-js`.
