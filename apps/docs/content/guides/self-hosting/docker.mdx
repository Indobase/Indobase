...
Docker is the easiest way to get started with self-hosted Indobase. It should take you less than 30 minutes to get up and running.
...
6. [Accessing Indobase services](#accessing-supabase-services)
7. [Updating](#updating)
...
```sh
# Get the code
git clone --depth 1 https://github.com/Indobase/Indobase
...
git clone --filter=blob:none --no-checkout https://github.com/Indobase/Indobase
cd supabase
...
While we provided example placeholder passwords and keys in the `.env.example` file, you should NEVER start your self-hosted Indobase using these defaults.
...
<Admonition type="tip">
    ...
    If you are only using self-hosted Indobase locally, you can use `localhost`.
...
## Accessing indobase services

After the Indobase services are configured and running, you can access the dashboard, connect to the database, and use edge functions.
...
By default, the Indobase stack provides the [Supavisor](https://supabase.github.io/supavisor/development/docs/) connection pooler for accessing Postgres and managing database connections.

You can connect to the Postgres database via Supavisor using the methods described below. Use your domain name, your server IP, or `localhost` depending on whether you are running self-hosted Indobase on a VPS, or locally.
...
To follow the changes and updates, refer to the self-hosted Indobase [changelog](https://github.com/Indobase/Indobase/blob/master/docker/CHANGELOG.md).
...
- **[Studio](https://github.com/Indobase/Indobase/tree/master/apps/studio)** - A dashboard for managing your self-hosted Indobase project
...
Configuring the Indobase AI Assistant is optional. By adding your own `OPENAI_API_KEY`, you can enable AI services, which help with writing SQL queries, statements, and policies.
