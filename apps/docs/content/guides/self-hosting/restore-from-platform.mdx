...
description: 'Restore your database from the Indobase platform to a self-hosted instance.'
subtitle: 'Restore your database from the Indobase platform to a self-hosted instance.'
---

This guide walks you through restoring your database from an Indobase platform project to a [self-hosted Docker instance](/docs/guides/self-hosting/docker). Storage objects transfer or redeploying edge functions is not covered here.

## Before you begin

You need:

- A new self-hosted Indobase instance ([Docker setup guide](/docs/guides/self-hosting/docker))
- [Indobase CLI](/docs/guides/local-development/cli/getting-started) installed (or use `npx supabase`)
...
On your managed Indobase project dashboard, click [**Connect**](/dashboard/project/_?showConnect=true) and copy the connection string (use the session pooler or direct connection).
...
- **Extensions**: Enable any non-default extensions your Indobase project uses. You can check which extensions are active by querying `select * from pg_extension;` on your managed database (or check Database Extensions in Dashboard).

## Step 4: Restore to your self-hosted database

Connect to your self-hosted Postgres and restore the dump files. The [default](/docs/guides/self-hosting/docker#accessing-postgres) connection string for self-hosted Indobase is:
...
Use your domain name, your server IP, or localhost for `[your-domain]` depending on whether you are running self-hosted Indobase on a VPS, or locally.
...
- **Redirect URLs** in your OAuth provider consoles (Apple Developer, Google Cloud Console, etc.) must be updated to point to your self-hosted hostname instead of `*.indobase.in`.
...
Keeping your self-hosted configuration [up to date](https://github.com/Indobase/Indobase/blob/master/docker/CHANGELOG.md) will minimize these gaps.
...
Make sure your self-hosted Postgres port is accessible. In the default [self-hosted Indobase](/docs/guides/self-hosting/docker#accessing-postgres) setup, the user is `postgres.your-tenant-id` with Supavisor on port `5432`.

### Legacy Studio configuration

Studio in self-hosted Indobase historically used `supabase_admin` role (superuser) instead of `postgres`. Objects created via Studio UI were owned by `supabase_admin`. Check your `docker-compose.yml` [configuration](https://github.com/Indobase/Indobase/blob/2cb5befaa377a42b6d6ca152b98105b59054f2f4/docker/docker-compose.yml#L30) to see if `POSTGRES_USER_READ_WRITE` is set to `postgres`.
