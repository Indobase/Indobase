...
Realtime doesn't log new WebSocket connections or Channel joins by default. Enable connection logging per client by including an `info` `log_level` parameter when instantiating the Indobase client.
...
const supabase = createClient('https://[PROJECT-REF].indobase.in', 'publishable-or-anon-key', options)
```

## Logs Explorer

The [Logs Explorer](/dashboard/project/_/logs-explorer) exposes logs from each part of the Indobase stack as a separate table that can be queried and joined using SQL.
