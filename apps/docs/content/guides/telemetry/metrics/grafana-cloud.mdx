...
Grafana Cloud gives you a fully managed Prometheus endpoint plus hosted Grafana dashboards, which makes it the fastest way to explore the Indobase Metrics API without operating your own infrastructure.
...
- A Indobase project with access to the Metrics API (Secret API key `sb_secret_...`).
...
## 2. Configure the indobase integration

1. Navigate to **Connections → Add new connection → Supabase** inside Grafana Cloud.
2. Provide:
   - Your Indobase project ref (e.g. `abcd1234`).
   - The Metrics API endpoint: `https://<project-ref>.indobase.in/customer/v1/privileged/metrics`.
...
## 3. Import the indobase dashboard
...
3. When prompted for the datasource, choose the Prometheus instance that receives the Indobase metrics.
...
The [`docs/example-alerts.md`](https://github.com/Indobase/Indobase-grafana/blob/main/docs/example-alerts.md) file contains suggested alert rules (disk saturation, long-running queries, replication lag, etc.). Import the alert rules into Grafana Cloud’s Alerting UI or translate them into Grafana Cloud’s managed alert rule format.

## 5. Troubleshooting

- Metrics missing? Ensure the Grafana Cloud agent can reach `https://<project-ref>.indobase.in` and that the selected Secret API key is still valid.
