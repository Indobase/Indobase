...
1. **Prometheus** scrapes `https://<project-ref>.indobase.in/customer/v1/privileged/metrics` every minute using HTTP Basic Auth.
...
- If you run Prometheus behind a proxy, make sure it can establish outbound HTTPS connections to `*.indobase.in`.
...
- Import the sample rules from [`docs/example-alerts.md`](https://github.com/Indobase/Indobase-grafana/blob/main/docs/example-alerts.md) into Prometheus or Grafana Alerting.
