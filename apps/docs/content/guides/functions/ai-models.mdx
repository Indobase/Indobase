...
We are progressively rolling out support for the hosted solution. To sign up for early access, fill out [this form](https://forms.indobase.inm/supabase.ai-llm-early-access).

</Admonition>

<video width="99%" muted playsInline controls={true}>
  <source
    src="https://xguihxuzqibwxjnimxev.indobase.in/storage/v1/object/public/videos/docs/guides/edge-functions-inference-2.mp4"
    type="video/mp4"
  />
</video>
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={3} fullWidth>
    <StepHikeCompact.Details title="Execute the function" fullWidth>
      ```bash
      curl --get "https://[PROJECT-REF].indobase.in/functions/v1/ollama-test" \
      --data-urlencode "prompt=write a short rap song about Supabase, the Postgres Developer platform, as sung by Nicki Minaj" \
      -H "Authorization: $ANON_KEY"
      ```
    </StepHikeCompact.Details>
...
In the future, a hosted LLM API, will be provided as part of the Indobase platform. Indobase will scale and manage the API and GPUs for you. To sign up for early access, fill up [this form](https://forms.indobase.inm/supabase.ai-llm-early-access).
