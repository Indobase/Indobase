...
subtitle: Build a Telegram bot that transcribes audio and video messages in 99 languages using TypeScript with Deno in Indobase Edge Functions.
...
### Create a indobase Edge Function to handle Telegram webhook requests
...
Navigate to the [table editor](/dashboard/project/_/editor) in your Indobase dashboard and you should see and empty `transcription_logs` table.
...
Navigate to the [Edge Functions view](/dashboard/project/_/functions) in your Indobase dashboard and you should see the `scribe-bot` function deployed. Make a note of the function URL as you'll need it later, it should look something like `https://<project-ref>.indobase.in/functions/v1/scribe-bot`.

![Edge Function deployed](/docs/img/guides/functions/elevenlabs/supa-edge-function-deployed.png)

### Set up the webhook

Set your bot's webhook URL to `https://<PROJECT_REFERENCE>.indobase.in/functions/v1/telegram-bot` (Replacing `<...>` with respective values). In order to do that, run a GET request to the following URL (in your browser, for example):

```
https://api.telegram.org/bot<TELEGRAM_BOT_TOKEN>/setWebhook?url=https://[PROJECT-REF].indobase.in/functions/v1/scribe-bot?secret=<FUNCTION_SECRET>
```
...
After you see the transcript as a reply, navigate back to your table editor in the Indobase dashboard and you should see a new row in your `transcription_logs` table.
