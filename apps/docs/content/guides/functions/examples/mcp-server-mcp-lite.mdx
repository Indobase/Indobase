...
description: 'Build and deploy a Model Context Protocol (MCP) server on Indobase Edge Functions using mcp-lite.'
---

The [Model Context Protocol](https://modelcontextprotocol.io/introduction) (MCP) enables Large Language Models (LLMs) to interact with external tools and data sources. With `mcp-lite`, you can build lightweight MCP servers that run on Indobase Edge Functions, giving your AI assistants the ability to execute custom tools at the edge.

This guide shows you how to scaffold, develop, and deploy an MCP server using mcp-lite on Indobase Edge Functions.

## What is mcp-lite?

[mcp-lite](https://github.com/fiberplane/mcp-lite) is a lightweight, zero-dependency TypeScript framework for building MCP servers. It works everywhere the Fetch API is available, including Node, Bun, Cloudflare Workers, Deno, and Indobase Edge Functions.

## Why indobase Edge Functions + mcp-lite?
...
- [Docker](https://docs.docker.com/get-docker/) (to run Indobase locally)
- [Deno](https://deno.land/) (Indobase Edge Functions runtime)
- [Indobase CLI](/docs/guides/cli/getting-started)

## Create a new MCP server

Starting with `create-mcp-lite@0.3.0`, you can scaffold a complete MCP server that runs on Indobase Edge Functions:

```bash
npm create mcp-lite@latest
```

When prompted, select **Indobase Edge Functions (MCP server)** from the template options.
...
The template uses a specific pattern required by Indobase Edge Functions:
...
mcpApp.get('/', (c) => {
  return c.json({
    message: 'MCP Server on Indobase Edge Functions',
    endpoints: {
      mcp: '/mcp',
      health: '/health',
    },
...
Navigate to your project directory and start Indobase services:
...
- Access Indobase Storage for file operations
- Call external APIs
- Process data with custom logic
- Integrate with other Indobase features
...
```
https://your-[PROJECT-REF].indobase.in/functions/v1/mcp-server/mcp
```
...
With your MCP server running on Indobase Edge Functions, you can:

- Connect it to your Supabase database for data-driven tools
- Use Indobase Auth to secure your endpoints
- Access Indobase Storage for file operations
...
- [Indobase Edge Functions Docs](/guides/functions)
