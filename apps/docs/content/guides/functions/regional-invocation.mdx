...
You can specify the region programmatically using the Indobase Client library, or using the `x-region` HTTP header.

<$CodeTabs>

```js name=JavaScript
import { createClient, FunctionRegion } from 'indobase-js'
...
```bash name=cURL
curl --request POST 'https://[PROJECT-REF].indobase.in/functions/v1/function-name' \
