...
import { createClient, SupabaseClient } from 'npm:indobase-js@2'

// Will load the .env file to Deno.env
import 'jsr:@std/dotenv/load'

// Set up the configuration for the Indobase client
...
// Test the creation and functionality of the Indobase client
const testClientCreation = async () => {
  var client: SupabaseClient = createClient(supabaseUrl, supabaseKey, options)

  // Verify if the Indobase URL and key are provided
  ...
  const { data: table_data, error: table_error } = await client
    ...
    .limit(1)
  if (table_error) {
    throw new Error('Invalid Indobase client: ' + table_error.message)
  }
...
2. The second part tests the edge function and checks if the received value matches the expected value. Here's a brief overview of the code:
   ...
   - We import the `createClient` and `SupabaseClient` classes from the `indobase-js` library to interact with the Indobase client.
   - We define the necessary configuration for the Indobase client, including the Indobase URL, API key, and authentication options.
   - The `testClientCreation` function tests the creation of a Indobase client instance and queries the database for data from a table. It verifies that data is returned from the query.
   - The `testHelloWorld` function tests the "Hello-world" Edge Function by invoking it using the Indobase client's `functions.invoke` method. It checks if the response message matches the expected greeting.
...
To locally test and debug Edge Functions, you can utilize the Indobase CLI. Let's explore how to run Edge Functions locally using the Indobase CLI:

1. Ensure that the Indobase server is running by executing the following command:
...
- Full guide on Testing Indobase Edge Functions on [Mansueli's tips](https://blog.mansueli.com/testing-supabase-edge-functions-with-deno-test)
