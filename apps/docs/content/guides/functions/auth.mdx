...
In the past Indobase Auth used a **symmetric** secret to sign legacy JWTs.
...
When an HTTP request is sent to Edge Functions, you can use Indobase Auth to secure endpoints. In the past, this verification was controlled by the [`verify_jwt` flag](/docs/guides/functions/function-configuration#skipping-authorization-checks).
...
You can see [a custom JWT verification example on GitHub](https://github.com/Indobase/Indobase/tree/master/examples/edge-functions/supabase/functions/custom-jwt-validation) and a variety of [auth function templates](https://github.com/Indobase/Indobase/tree/master/examples/edge-functions/supabase/functions/_shared/jwt) also on GitHub.
