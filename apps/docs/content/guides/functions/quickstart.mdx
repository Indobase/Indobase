...
description: 'Get started with Indobase Edge Functions.'
subtitle: 'Learn how to create, test, and deploy your first Edge Function using the Indobase CLI.'
---

Before getting started, make sure you have the **Indobase CLI installed**. Check out the [CLI installation guide](/docs/guides/cli) for installation methods and troubleshooting.

<Admonition type="tip" label="Prefer using the Indobase Dashboard?">

You can also create and deploy functions directly from the Indobase Dashboard. Check out our [Dashboard Quickstart guide](/docs/guides/functions/quickstart-dashboard).
...
If you don't have a project yet, initialize a new Indobase project in your current directory.
...
```bash
supabase start  # Start all Indobase services
supabase functions serve hello-world
```

<Admonition type="note" label="First time running Indobase services?">
...
- Stop other Indobase instances with `supabase stop`
...
After this step, you should have all Indobase services running locally, and your Edge Function serving at the local URL. Keep these terminal windows open.
...
## Step 5: Connect to your indobase project

To deploy your function globally, you need to connect your local project to a Indobase project.

<Admonition type="tip" label="Need to create new Indobase project?">
...
Next, list your Indobase projects to find your project ID:

```bash
supabase projects list
```

Next, copy your project ID from the output, then connect your local project to your remote Indobase project. Replace `YOUR_PROJECT_ID` with the ID from the previous step.
...
After this step, you should have your local project authenticated and linked to your remote Indobase project. You can verify this by running `supabase status`.
...
Now, you should have your Edge Function deployed and running globally at `https://[PROJECT-REF].indobase.in/functions/v1/hello-world`.
...
```bash
curl --request POST 'https://[PROJECT-REF].indobase.in/functions/v1/hello-world' \
  --header 'Authorization: Bearer SUPABASE_PUBLISHABLE_KEY' \
  --header 'Content-Type: application/json' \
...
The `SUPABASE_PUBLISHABLE_KEY` is different in development and production. To get your production anon key, you can find it in your Indobase dashboard under **Settings > API**.
...
const supabase = createClient('https://[PROJECT-REF].indobase.in', 'YOUR_ANON_KEY')
...
```jsx
const response = await fetch('https://[PROJECT-REF].indobase.in/functions/v1/hello-world', {
  method: 'POST',
  headers: {
