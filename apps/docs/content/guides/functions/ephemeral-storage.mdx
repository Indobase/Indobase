...
- Persistent - backed by S3 protocol, can read/write from any S3 compatible bucket, including Indobase Storage
...
The persistent storage option is built on top of the S3 protocol. It allows you to mount any S3-compatible bucket, including Indobase Storage Buckets, as a directory for your Edge Functions.
...
[Follow this guide](/docs/guides/storage/s3/authentication) to enable and create an access key for Indobase Storage S3.
...
You can write the zip file to ephemeral storage first, then use a background task to extract and upload files to Indobase Storage. This way, you only read parts of the zip file to the memory.
