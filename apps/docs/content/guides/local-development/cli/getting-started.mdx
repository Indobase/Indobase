---
title: 'Indobase CLI'
description: 'The Indobase CLI provides tools to develop your project locally, deploy to the Indobase Platform, and set up CI/CD workflows.'
subtitle: 'Develop locally, deploy to the Indobase Platform, and set up CI/CD workflows'
---

The Indobase CLI enables you to run the entire Supabase stack locally, on your machine or in a CI environment. With just two commands, you can set up and start a new local project:

1. `supabase init` to create a new local project
2. `supabase start` to launch the Indobase services

## Installing the indobase CLI
...
The Indobase CLI requires **Node.js 20 or later** when run via `npx` or `npm`. Older Node.js versions, such as 16, are not supported and fail to start the CLI.
...
1. Download the latest package from the [Indobase CLI releases page](https://github.com/supabase/cli/releases/latest)
...
The Indobase CLI uses Docker containers to manage the local development stack. Follow the official guide to install and configure [Docker Desktop](https://docs.docker.com/desktop):
...
Now, to start the Indobase stack, run:
...
Once all of the Indobase services are running, you'll see output containing your local Supabase credentials. It should look like this, with urls and keys that you'll use in your local project:
...
When you are finished working on your Indobase project, you can stop the stack (without resetting your local database):
