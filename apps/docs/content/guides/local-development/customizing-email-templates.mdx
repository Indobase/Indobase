...
```
https://[PROJECT-REF].indobase.in/auth/v1/verify?token={{ .TokenHash }}&type=email&redirect_to=https://example.com/path
```
...
These settings are for local development. To apply the changes locally, stop and restart the Indobase containers:
