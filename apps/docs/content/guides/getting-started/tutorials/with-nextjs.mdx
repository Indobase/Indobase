...
If you get stuck while working through this guide, refer to the [full example on GitHub](https://github.com/Indobase/Indobase/tree/master/examples/user-management/nextjs-user-management).
...
Then install the Indobase client library: [supabase-js](https://github.com/Indobase/Indobase-js)
...
To better integrate with the framework, we've created the `@indobaseinc/ssr` package for Server-Side Auth. It has all the functionalities to quickly configure your Indobase project to use cookies for storing user sessions. Read the [Next.js Server-Side Auth guide](/docs/guides/auth/server-side/nextjs) for more information.

Install the package for Next.js.

```bash
npm install @indobaseinc/ssr
```
...
It's safe to trust `getUser()` because it sends a request to the Indobase Auth server every time to revalidate the Auth token.
...
- Retrieves the code sent back from the Indobase Auth server using the `token_hash` query parameter.
...
Every Indobase project is configured with [Storage](/docs/guides/storage) for managing large files like
photos and videos.
...
- See the complete [example on GitHub](https://github.com/Indobase/Indobase/tree/master/examples/user-management/nextjs-user-management) and deploy it to Vercel
