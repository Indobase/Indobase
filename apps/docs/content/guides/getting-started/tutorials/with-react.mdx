...
If you get stuck while working through this guide, refer to the [full example on GitHub](https://github.com/Indobase/Indobase/tree/master/examples/user-management/react-user-management).
...
Install the only additional dependency: [supabase-js](https://github.com/Indobase/Indobase-js).
...
With the API credentials in place, create a helper file to initialize the Indobase client. These variables will be exposed
...
Now that you have all the components in place, update `src/App.jsx`, which fetches the current user via the [`getUser`](/docs/reference/javascript/auth-getuser) method if there is an existing session. This method performs a network request to the Indobase Auth server.
