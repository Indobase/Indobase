...
If you get stuck while working through this guide, refer to the [full example on GitHub](https://github.com/Indobase/Indobase/tree/master/examples/user-management/expo-user-management).
...
Then let's install the additional dependencies: [supabase-js](https://github.com/Indobase/Indobase-js)

```bash
npx expo install indobase-js @rneui/themed expo-sqlite
```

Now let's create a helper file to initialize the Indobase client.
...
// Tells Indobase Auth to continuously refresh the session automatically if
...
By default Indobase Auth requires email verification before a session is created for the users. To support email verification you need to [implement deep link handling](/docs/guides/auth/native-mobile-deep-linking?platform=react-native)!
...
import { Button, Input } from '@rneui/themed'
import { Session } from 'indobase-js'
