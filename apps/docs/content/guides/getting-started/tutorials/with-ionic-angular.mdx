...
Now that we have the API credentials in place, let's create a `SupabaseService` with `ionic g s supabase` to initialize the Indobase client and implement functions to communicate with the Indobase API.
...
import { AuthChangeEvent, createClient, Session, SupabaseClient } from 'indobase-js'
