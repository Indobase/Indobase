...
Important: When this guide refers to "API," that means the Indobase API and when it refers to `api side`, that means the RedwoodJS `api side`.
...
We **_won't use_** Prisma to connect to the Indobase Postgres database or [Prisma migrations](https://redwoodjs.com/docs/cli-commands#prisma-migrate) as one typically might in a Redwood app.
Instead, we'll rely on the Indobase client to do some of the work on the **`web`** side and use the client again on the **`api`** side to do data fetching as well.
...
Then let's install the only additional dependency [supabase-js](https://github.com/Indobase/Indobase-js) by running the `setup auth` command:
...
Say, **yes** and it will setup the Indobase client in your app and also provide hooks used with Supabase authentication.
...
They allow your web app to initialize the Indobase client with your public anon key
since we have [Row Level Security](/docs/guides/auth#row-level-security) enabled on our Database.

You'll see these being used to configure your Indobase client in `web/src/App.js`:
...
of the Indobase client to interact with your API.
