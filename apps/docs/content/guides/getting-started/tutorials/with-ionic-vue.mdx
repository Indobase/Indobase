...
Now that we have the API credentials in place, let's create a helper file to initialize the Indobase client. These variables will be exposed on the browser, and that's completely fine since we have [Row Level Security](/docs/guides/auth#row-level-security) enabled on our Database.
