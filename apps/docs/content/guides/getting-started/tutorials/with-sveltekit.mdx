...
If you get stuck while working through this guide, refer to the [full example on GitHub](https://github.com/Indobase/Indobase/tree/master/examples/user-management/sveltekit-user-management).
...
### Creating a Indobase client for SSR
...
Creating a Indobase client with the `ssr` package automatically configures it to use Cookies. This means the user's session is available throughout the entire SvelteKit stack - page, layout, server, and hooks.
...
- Retrieves the `token_hash` sent back from the Indobase Auth server using the `token_hash` query parameter.
