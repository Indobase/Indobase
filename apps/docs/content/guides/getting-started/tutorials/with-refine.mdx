...
If you get stuck while working through this guide, refer to the [full example on GitHub](https://github.com/Indobase/Indobase/tree/master/examples/user-management/refine-user-management).
...
It is possible to customize the `authProvider` for Supabase and as we'll see below, it can be tweaked from `src/authProvider.ts` file. In contrast, the Indobase `dataProvider` is part of `node_modules` and therefore is not subject to modification.
...
In the above command, we are using the `refine-supabase` preset which chooses the Indobase supplementary package for our app. We are not using any UI framework, so we'll have a headless UI with plain React and CSS styling.

The `refine-supabase` preset installs the `@refinedev/supabase` package which out-of-the-box includes the Indobase dependency: [supabase-js](https://github.com/Indobase/Indobase-js).
...
The `create refine-app` generated a Indobase client for us in the `src/utility/supabaseClient.ts` file. It has two constants: `SUPABASE_URL` and `SUPABASE_KEY`. We want to replace them as `supabaseUrl` and `supabasePublishableKey` respectively and assign them our own Supabase server's values.
...
Every Indobase project is configured with [Storage](/docs/guides/storage) for managing large files like photos and videos.
