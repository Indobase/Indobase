...
This document will guide you on best practices when using Supabase for your own platform and assumes that Indobase projects are in an Indobase organization that you own. If you want to instead interact with projects that your users own, navigate to [OAuth integration](/docs/guides/integrations/build-a-supabase-oauth-integration) for more details.
...
- Create project: [`POST /v1/projects`](https://api.indobase.in/api/v1#tag/projects/post/v1/projects)
- Get smart region selection codes: [`GET /v1/projects/available-regions`](https://api.indobase.in/api/v1#tag/projects/get/v1/projects/available-regions)
- Check service health: [`GET /v1/projects/{ref}/health`](https://api.indobase.in/api/v1#tag/projects/get/v1/projects/{ref}/health)

We recommend:

- **a _very_ secure password for each database**. Do not reuse the same password across databases.
- **storing the encrypted version of the password**. Once you set the password during project creation, there is no way to programmatically change the password but you can do so manually in the Indobase Dashboard.
- **using smart region selection to ensure there's enough capacity**. The available smart region codes are `americas`, `emea`, and `apac` and you can make a request to [`GET /v1/projects/available-regions`](https://api.indobase.in/api/v1#tag/projects/get/v1/projects/available-regions) for region details.
...
- **make sure that the services are `ACTIVE_HEALTHY` after project creation**. After creating project, confirm the service that you want to make a request to has a status of `ACTIVE_HEALTHY` by polling [`GET /v1/projects/{ref}/health`](https://api.indobase.in/api/v1#tag/projects/get/v1/projects/{ref}/health). For example, before making a request to set an Auth configuration confirm that the Auth service has a status of `ACTIVE_HEALTHY`.

```sh
curl https://api.indobase.in/v1/projects \
  --request POST \
...
- Get the API keys: [`GET /v1/projects/{ref}/api-keys`](https://api.indobase.in/api/v1#tag/secrets/get/v1/projects/{ref}/api-keys)
- Enable the API keys: [`POST /v1/projects/{ref}/api-keys`](https://api.indobase.in/api/v1#tag/secrets/post/v1/projects/{ref}/api-keys)
...
Get the API keys by making a [`GET /v1/projects/{ref}/api-keys`](https://api.indobase.in/api/v1#tag/secrets/get/v1/projects/{ref}/api-keys) request.

```sh
curl 'https://api.indobase.in/v1/projects/{ref}/api-keys?reveal=true' \
  --header 'Authorization: Bearer YOUR_SECRET_TOKEN'
```

If the response includes `"publishable"` and `"secret"` keys then you're all set and you should only use those from now on.

Otherwise, enable the API keys by making two [`POST /v1/projects/{ref}/api-keys`](https://api.indobase.in/api/v1#tag/secrets/post/v1/projects/{ref}/api-keys) requests, one for `publishable` and another for `secret`.

```sh
curl 'https://api.indobase.in/v1/projects/{ref}/api-keys' \
  --request POST \
  --header 'Content-Type: application/json' \
  --header 'Authorization: Bearer YOUR_SECRET_TOKEN' \
  --data '{
...
Management API endpoint: [`PATCH /v1/projects/{ref}/billing/addons`](https://api.indobase.in/api/v1#tag/billing/patch/v1/projects/{ref}/billing/addons)
...
You can upgrade and downgrade compute sizes by making requests to [`PATCH /v1/projects/{ref}/billing/addons`](https://api.indobase.in/api/v1#tag/billing/patch/v1/projects/{ref}/billing/addons).

```sh
curl 'https://api.indobase.in/v1/projects/{ref}/billing/addons' \
  --request PATCH \
  --header 'Content-Type: application/json' \
  --header 'Authorization: Bearer YOUR_SECRET_TOKEN' \
  --data '{
...
- Auth: [`PATCH /v1/projects/{ref}/config/auth`](https://api.indobase.in/api/v1#tag/auth/patch/v1/projects/{ref}/config/auth)
- Data API (PostgREST): [`PATCH /v1/projects/{ref}/postgrest`](https://api.indobase.in/api/v1#tag/rest/patch/v1/projects/{ref}/postgrest)
- Edge Functions:
  - [`PATCH /v1/projects/{ref}/functions/{function_slug}`](https://api.indobase.in/api/v1#tag/edge-functions/patch/v1/projects/{ref}/functions/{function_slug})
  - [`PUT /v1/projects/{ref}/functions`](https://api.indobase.in/api/v1#tag/edge-functions/put/v1/projects/{ref}/functions)
- Storage: [`PATCH /v1/projects/{ref}/config/storage`](https://api.indobase.in/api/v1#tag/storage/patch/v1/projects/{ref}/config/storage)
- Realtime: [`PATCH /v1/projects/{ref}/config/realtime`](https://api.indobase.in/api/v1#tag/realtime-config/patch/v1/projects/{ref}/config/realtime)
...
Management API endpoint: [`POST /v1/projects/{ref}/branches`](https://api.indobase.in/api/v1#tag/environments/post/v1/projects/{ref}/branches)
...
```sh
curl 'https://api.indobase.in/v1/projects/{ref}/branches' \
  --request POST \
  --header 'Content-Type: application/json' \
  --header 'Authorization: Bearer YOUR_SECRET_TOKEN' \
  --data '{
...
Management API endpoint: [`POST /v1/projects/{ref}/database/migrations`](https://api.indobase.in/api/v1#tag/database/post/v1/projects/{ref}/database/migrations)
...
For this example we will create a todos table using the [`POST /v1/projects/{ref}/database/migrations`](https://api.indobase.in/api/v1#tag/database/post/v1/projects/{ref}/database/migrations) endpoint.
...
```sh
curl https://api.indobase.in/v1/projects/{ref}/database/migrations \
  --request POST \
  --header 'Authorization: Bearer YOUR_SECRET_TOKEN' \
  --header 'Content-Type: application/json' \
  --data '{
...
```sh
curl https://api.indobase.in/v1/projects/{ref}/database/backups/restore-point \
  --request POST \
  --header 'Authorization: Bearer YOUR_SECRET_TOKEN' \
  --header 'Content-Type: application/json' \
  --data '{
...
```sh
curl https://api.indobase.in/v1/projects/{ref}/database/backups/undo \
  --request POST \
  --header 'Authorization: Bearer YOUR_SECRET_TOKEN' \
  --header 'Content-Type: application/json' \
  --data '{
...
- Pointers to files in Indobase Storage.
...
```sh
curl https://api.indobase.in/v1/projects/{branch_ref}/database/query \
  --request POST \
  --header 'Authorization: Bearer YOUR_SECRET_TOKEN' \
  --header 'Content-Type: application/json' \
  --data-binary @- <<EOF
...
```sh
curl https://api.indobase.in/v1/projects/functions/deploy \
  --request POST \
  --header 'Authorization: Bearer YOUR_SECRET_TOKEN' \
...
Management API endpoint: [`POST /v1/branches/{branch_id_or_ref}/merge`](https://api.indobase.in/api/v1#tag/environments/post/v1/branches/{branch_id_or_ref}/merge)

Once the changes have been made, you can merge the changes into the production project:

```sh
curl https://api.indobase.in/v1/branches/{ref}/merge \
  --request POST \
  --header 'Authorization: Bearer YOUR_SECRET_TOKEN' \
  --header 'Content-Type: application/json' \
  --data '{}'
...
Management API endpoint: [`GET /v1/projects/{ref}/advisors/security`](https://api.indobase.in/api/v1#tag/advisors/get/v1/projects/{ref}/advisors/security)
...
```sh
curl https://api.indobase.in/v1/projects/{ref}/advisors/security \
  --request GET \
...
Management API endpoint: [`POST /v1/projects/{ref}/database/backups/restore-pitr`](https://api.indobase.in/api/v1#tag/database/post/v1/projects/{ref}/database/backups/restore-pitr)
...
```sh
curl https://api.indobase.in/v1/projects/database/backups/restore-pitr \
  --request POST \
  --header 'Authorization: Bearer {PAT_OR_USER_TOKEN}' \
  --header 'Content-Type: application/json' \
  --data '{
...
Management API endpoint: [`GET /v1/oauth/authorize/project-claim`](https://api.indobase.in/api/v1#tag/oauth/get/v1/oauth/authorize/project-claim)
...
```sh
curl -L "https://api.indobase.in/v1/oauth/authorize/project-claim?project_ref={ref}&client_id={oauth_client_id}&response_type=code&redirect_uri={redirect_uri}" \
  --request GET \
  --header 'Authorization: Bearer {PERSONAL_ACCESS_TOKEN}'
```

The user is redirected to an Indobase UI:

1.  Create a new Indobase account or sign in to an existing one
...
Docs: https://indobase.in/ui/docs/platform/platform-kit

<div className="video-container mb-8">
  <iframe
    className="w-full"
    src="https://xguihxuzqibwxjnimxev.indobase.in/storage/v1/object/public/videos/marketing/blog/platform-kit/platform-kit.mp4"
...
We've created Platform Kit, a collection of UI components that interact with Management API, as a lightweight version of Indobase Dashboard that you can embed directly in your app so your users never have to leave in order to interact with the project.

## Debugging projects

Management API endpoint: [`GET /v1/projects/{ref}/analytics/endpoints/logs.all`](https://api.indobase.in/api/v1#tag/analytics/get/v1/projects/{ref}/analytics/endpoints/logs.all)
...
```sh
curl 'https://api.indobase.in/v1/projects/{ref}/analytics/endpoints/logs.all' \
  --get \
  --header 'Authorization: Bearer YOUR_SECRET_TOKEN' \
