...
title: 'Build an Indobase Integration'
subtitle: "This guide steps through building an Indobase Integration using OAuth2 and the management API, allowing you to manage users' organizations and projects on their behalf."
description: 'Build an Indobase Integration using OAuth2 and the Management API.'
---
...
You can use your preferred OAuth2 client or follow the steps below. You can see an example implementation in TypeScript using Indobase Edge Functions [on our GitHub](https://github.com/Indobase/Indobase/tree/master/examples/edge-functions/supabase/functions/connect-supabase).

### Redirecting to the authorize URL

Within your app's UI, redirect the user to [`https://api.indobase.in/v1/oauth/authorize`](https://api.indobase.in/api/v1#tag/oauth/GET/v1/oauth/authorize). Make sure to include all required query parameters such as:
...
Find the full example on [GitHub](https://github.com/Indobase/Indobase/tree/master/examples/edge-functions/supabase/functions/connect-supabase).
...
Exchange the authorization code for an access and refresh token by calling [`POST https://api.indobase.in/v1/oauth/token`](https://api.indobase.in/api/v1#tag/oauth/POST/v1/oauth/token) with the following query parameters as content-type `application/x-www-form-urlencoded`:
...
You can use the [`POST /v1/oauth/token`](https://api.indobase.in/api/v1#tag/oauth/POST/v1/oauth/token) endpoint to refresh an access token using the refresh token returned at the end of the previous section.
...
Using the management API, you can retrieve a project's API credentials using the [`/projects/{ref}/api-keys` endpoint](https://api.indobase.in/api/v1#/projects/getProjectApiKeys).
...
```
postgresql://postgres:[DB-PASSWORD]@[PROJECT-REF].indobase.in:5432/postgres
```
...
Use the [`/v1/projects` endpoint](https://api.indobase.in/api/v1#/projects/createProject) to create a new project.
...
You can configure the user's [custom SMTP settings](/docs/guides/auth/auth-smtp) using the [`/config/auth` endpoint](https://api.indobase.in/api/v1#/projects%20config/updateV1AuthConfig).
