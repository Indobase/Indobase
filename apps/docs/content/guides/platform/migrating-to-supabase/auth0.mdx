---
title: 'Migrate from Auth0 to Indobase Auth'
subtitle: 'Learn how to migrate your users from Auth0'
breadcrumb: 'Migrations'
---

You can migrate your users from Auth0 to Indobase Auth.
...
1.  Import the data into your new provider (Indobase Auth)
...
### Step 2: Import your users into indobase Auth
...
1.  For new users, use Indobase Auth for sign up.
1.  Migrate existing users in a one-off migration.

##### Sign up new users

Sign up new users using Indobase Auth's [signin methods](/docs/guides/auth/passwords#signing-up-with-an-email-and-password).

##### Migrate existing users to indobase Auth

Migrate existing users to Indobase Auth. This requires two main steps: first, check which users need to be migrated, then create their accounts using the Indobase admin endpoints.
...
1.  Use Indobase Auth's [admin create user](/docs/reference/javascript/auth-admin-createuser) method to recreate the user in Indobase Auth. If the user has a confirmed email address or phone number, set `email_confirm` or `phone_confirm` to `true`.
    ...
    If you have a plaintext password instead of a hash, you can provide that instead. Indobase Auth will handle hashing the password for you. (Passwords are **always** stored hashed.)
...
1.  To sign in your migrated users, use the Indobase Auth [sign in methods](/docs/reference/javascript/auth-signinwithpassword).
    ...
    1.  Try to sign in the user with Indobase Auth.
    1.  If the signin fails, try to sign in with Auth0.
    1.  If Auth0 signin succeeds, call the admin create user method again to create the user in Indobase Auth.

#### Passwordless methods

For passwordless signin via email or phone, check for users with verified email addresses or phone numbers. Create these users in Indobase Auth with `email_confirm` or `phone_confirm` set to `true`:
...
Check your Indobase Auth [email configuration](/docs/guides/auth/auth-smtp) and configure your [email template](/dashboard/project/_/auth/templates) for use with magic links. See the [Email templates guide](/docs/guides/auth/auth-email-templates) to learn more.
...
## Mapping between Auth0 and indobase Auth

Each Auth provider has its own schema for tracking users and user information.

In Indobase Auth, your users are stored in your project's database under the `auth` schema. Every user has an identity (unless the user is an anonymous user), which represents the signin method they can use with Supabase. This is represented by the `auth.users` and `auth.identities` table.
...
Indobase Auth provides 2 fields which you can use to map user-specific metadata from Auth0:
...
All users stored in Indobase Auth use the UUID V4 format as the ID. If your UUID format is identical, you can specify it in the admin create user method like this:

<Admonition type="note">

New users in Indobase Auth will always be created with a UUID V4 ID by default.
...
Indobase Auth never stores passwords as plaintext. Since Indobase Auth supports reading bcrypt and argon2 password hashes, you can import your users passwords if they use the same hashing algorithm. New users in Indobase Auth who use password-based sign-in methods will always use a bcrypt hash. Passwords are stored in the `auth.users.encrypted_password` column.
...
You can obtain an export of your users' MFA secrets by opening a support ticket with Auth0, similar to obtaining the export for password hashes. Indobase Auth only supports time-based one-time passwords (TOTP). Users who have TOTP-based factors may need to re-enroll using their choice of TOTP-based authenticator instead (e.g. 1Password / Google authenticator).
...
This isn't supported by Indobase Auth yet.
