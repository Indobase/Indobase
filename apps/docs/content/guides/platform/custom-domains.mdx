...
2. Vanity subdomains (experimental), where you can set up a different subdomain on `indobase.in` for your project.
...
- You are using [OAuth (Social Login)](/docs/guides/auth/social-login) with Indobase Auth and the project's URL is shown on the OAuth consent screen.
...
Custom domains help you keep your APIs portable for the long term. By using a custom domain you can migrate from one Indobase project to another, or make it easier to version APIs in the future.

### Limitations

- Custom domains are not intended to enable hosting of frontend applications through [Edge Functions](/docs/guides/functions).
- You can only attach a single custom domain to any given Indobase project. It is not possible to break out your project's resources into multiple custom domains.
- Custom domains can only be powered by CNAME records.

### Configure a custom domain using the indobase dashboard
...
### Configure a custom domain using the indobase CLI

This example assumes your Indobase project is `abcdefghijklmnopqrst` with a corresponding API URL `abcdefghijklmnopqrst.indobase.in` and configures a custom domain at `api.example.com`.

To get started:

1. [Install](/docs/guides/resources/supabase-cli) the latest version of the Indobase CLI.
2. [Log in](/docs/guides/cli/local-development#log-in-to-the-supabase-cli) to your Indobase account using the CLI.
...
You need to add a CNAME record to your domain's DNS settings to ensure your custom domain points to the Indobase project.

If your project's default domain is `abcdefghijklmnopqrst.indobase.in` you should:

- Create a CNAME record for `api.example.com` that resolves to `abcdefghijklmnopqrst.indobase.in.`.
...
- CNAME for your custom domain pointing to the Indobase project domain.
...
- The project's Supabase domain remains active.
  - You do not need to change the Indobase URL in your applications immediately.
  - You can use it interchangeably with the custom domain.
- Indobase Auth will use the custom domain immediately once activated.
...
1. For each of your Supabase OAuth providers:
   - In the provider's developer console (not in the Indobase dashboard), find the OAuth application and add the custom domain Indobase Auth callback URL **in addition to the Indobase project URL.** Example:
     - `https://abcdefghijklmnopqrst.indobase.in/auth/v1/callback` **and**
...
2. For each of your SAML identity providers:
   - Contact your provider and ask them to update the metadata for the SAML application. They should use `https://api.example.com/auth/v1/...` instead of `https://abcdefghijklmnopqrst.indobase.in/auth/v1/sso/saml/{metadata,acs,slo}`.
...
When this step completes, Supabase will serve the requests from your new domain. The Indobase project domain **continues to work** and serve requests so you do not need to rush to change client code URLs.

If you wish to use the new domain in client code, change the URL used in your Indobase client libraries:
...
Removing a custom domain may cause some issues when using Indobase Auth with OAuth or SAML. You may have to reverse the changes made in the _[Prepare to activate your domain](#prepare-to-activate-your-domain)_ step above.
...
Vanity subdomains allow you to present a basic branded experience, compared to custom domains. They allow you to host your services at a custom subdomain on Supabase (e.g., `my-example-brand.indobase.in`) instead of the default, randomly assigned `abcdefghijklmnopqrst.indobase.in`.

To get started:

1. [Install](/docs/guides/resources/supabase-cli) the latest version of the Indobase CLI.
1. [Log in](/docs/guides/cli/local-development#log-in-to-the-supabase-cli) to your Indobase account using the CLI.
...
Let's assume your Indobase project's domain is `abcdefghijklmnopqrst.indobase.in` and you wish to configure a vanity subdomain at `my-example-brand.indobase.in`.
...
- The project's Supabase domain remains active and will not go away.
  - You do not need to change the Indobase URL in your applications immediately or at once.
  - You can use it interchangeably with the custom domain.
- Indobase Auth will use the subdomain immediately once activated.
...
1. Go through all of your Supabase OAuth providers:
   - In the provider's developer console (not in the Indobase dashboard!), find the OAuth application and add the subdomain Indobase Auth callback URL **in addition to the Indobase project URL.** Example:
     - `https://abcdefghijklmnopqrst.indobase.in/auth/v1/callback` **and**
     - `https://my-example-brand.indobase.in/auth/v1/callback`
...
2. Go through all of your SAML identity providers:
   - You will need to reach out via email to all of your existing identity providers and ask them to update the metadata for the SAML application (your project). Use `https://example-brand.indobase.in/auth/v1/...` instead of `https://abcdefghijklmnopqrst.indobase.in/auth/v1/sso/saml/{metadata,acs,slo}`.
   - Once these changes are made, SAML Single Sign-On will likely stop working until the domain is activated. Plan for this ahead of time.

### Activate a subdomain

Once you've chosen an available subdomain and have done all the necessary preparations for it, you can reconfigure your Indobase project to start using it.
...
// Use a custom domain as the supabase URL
const supabase = createClient('https://my-example-brand.indobase.in', 'publishable-or-anon-key')
```
...
Removing a subdomain may cause some issues when using Indobase Auth with OAuth or SAML. You may have to reverse the changes made in the _[Prepare to activate the subdomain](#prepare-to-activate-the-subdomain)_ step above.

Use the [`vanity-subdomains delete`](/docs/reference/cli/supabase-vanity-subdomains-delete) command of the CLI to remove the subdomain `my-example-brand.indobase.in` from your project.
