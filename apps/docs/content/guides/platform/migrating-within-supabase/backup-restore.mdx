...
subtitle: 'Learn how to backup and restore projects using the Indobase CLI'
...
<StepHikeCompact>
  <StepHikeCompact.Step step={1}>
    <StepHikeCompact.Details title="Install the Indobase CLI" fullWidth>
    Install the [Indobase CLI](/docs/guides/local-development/cli/getting-started).
    ...
    <StepHikeCompact.Step step={5}>
      <StepHikeCompact.Details title="Restore your Project with the CLI" fullWidth>
        ...
        >
          ...
          You can restore the project using both the old and new project ref (the project ref is the value between "https://" and ".indobase.in" in the URL) instead of the URL.
          ...
          curl "https://api.indobase.in/v1/projects/$OLD_PROJECT_REF/pgsodium" \
            -H "Authorization: Bearer $SUPABASE_ACCESS_TOKEN" |
          curl "https://api.indobase.in/v1/projects/$NEW_PROJECT_REF/pgsodium" \
            -H "Authorization: Bearer $SUPABASE_ACCESS_TOKEN" \
            -X PUT --json @-
          ```
          </TabPanel>
...
If you were using Indobase CLI for managing migrations on your old database and would like to preserve the migration history in your newly restored project, you need to insert the migration records separately using the following commands.
...
If you have modified the `auth` and `storage` schemas in your old project, such as adding triggers or Row Level Security(RLS) policies, you have to restore them separately. The Indobase CLI can help you diff the changes to these schemas using the following commands.
...
```js
// npm install indobase-js@2
const { createClient } = require('indobase-js')

const OLD_PROJECT_URL = 'https://xxx.indobase.in'
const OLD_PROJECT_SERVICE_KEY = 'old-project-service-key-xxx'

const NEW_PROJECT_URL = 'https://yyy.indobase.in'
