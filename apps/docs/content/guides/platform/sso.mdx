...
- If an SSO user with the following email of `alice@foocorp.com` attempts to sign in with a GitHub account that uses the same email, a separate Indobase account is created and will not be linked to the SSO user's account.
