...
description: 'Learn about the available backup methods for your Indobase project.'
---
...
The Indobase team regularly monitors the status of backups. In case of any issues, you can [contact support](/dashboard/support/new). Also you can check out our [status page](https://status.indobase.in/) at any time.
...
curl -H "Authorization: Bearer $SUPABASE_ACCESS_TOKEN" \
  "https://api.indobase.in/v1/projects/$PROJECT_REF/database/backups"

# Restore from a PITR (not logical) backup (replace ISO timestamp with desired restore point)
curl -X POST "https://api.indobase.in/v1/projects/$PROJECT_REF/database/backups/restore-pitr" \
...
Databases larger than 15GB[^1], if they're on a recent build[^2] of the Indobase platform, get automatically transitioned[^3] to use daily physical backups. Physical backups are a more performant backup mechanism that lowers the overhead and impact on the database being backed up, and also avoids holding locks on objects in your database for a long period of time. While restores are unaffected, the backups created using this method cannot be downloaded from the Backups section of the dashboard.
...
You can refer to [an example PR](https://github.com/Indobase/Indobase/pull/28393/files) on how to update SQL code to use schema-qualified names.
