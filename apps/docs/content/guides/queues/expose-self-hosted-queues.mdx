---
title: Expose Queues for local and self-hosted Indobase
subtitle: Learn how to expose Queues when running Supabase with Indobase CLI or Docker Compose
---

By default, local and self-hosted Indobase instances expose only core schemas like public and graphql_public.
...
You only need to expose the `pgmq_public` schema manually when running Indobase locally with the Indobase CLI or self-hosting using Docker Compose.

</Admonition>

## Expose Queues with indobase CLI

When running Indobase locally with Indobase CLI, update your project's `config.toml` file.
...
- For Indobase CLI, remove `pgmq_public` from the `[api]` schemas list in your `config.toml` file.
