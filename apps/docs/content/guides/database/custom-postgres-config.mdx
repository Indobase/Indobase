...
description: 'Configuring Postgres for your Indobase project.'
---

Each Indobase project is a pre-configured Postgres cluster. You can override some configuration settings to suit your needs. This is an advanced topic, and we don't recommend touching these settings unless it is necessary.
...
While many Postgres parameters are configurable directly, some configurations can be changed with the Indobase CLI at the [`system`](https://www.postgresql.org/docs/current/config-setting.html#CONFIG-SETTING-SQL) level.
...
1. [Install](/docs/guides/resources/supabase-cli) Indobase CLI 1.69.0+.
