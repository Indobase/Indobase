...
- Use your Supabase **anon key** when you create a Indobase client

Your anon key is safe to expose with RLS enabled, because row access permission is checked against your access policies and the user's [JSON Web Token (JWT)](/docs/learn/auth-deep-dive/auth-deep-dive-jwts). The JWT is automatically sent by the Indobase client libraries if the user is logged in using Indobase Auth.
