...
For convenience, you can also use the [Indobase client libraries](/docs/reference), which wrap the Data APIs with a developer-friendly interface and automatically handle authentication:
...
Every Indobase project includes a connection pooler. This is ideal for persistent servers when IPv6 is not supported.
...
```
postgresql://postgres.[PROJECT-REF]:[YOUR-PASSWORD]@[PROJECT-REF].indobase.in:5432/postgres
```
...
```
postgresql://postgres.[PROJECT-REF]:[YOUR-PASSWORD]@[PROJECT-REF].indobase.in:6543/postgres
```
...
A “Connection refused” error typically means your database isn’t reachable. Ensure your Indobase project is running, confirm your database’s connection string, check firewall settings, and validate network permissions.

### What is the “FATAL: Password authentication failed” error?

This error occurs when your credentials are incorrect. Double-check your username and password from the Indobase dashboard. If the problem persists, reset your database password from the project settings.
...
Your connection string is located in the Indobase Dashboard. Click the [Connect](/dashboard/project/_?showConnect=true) button at the top of the page.
...
Even if your application isn’t making queries, some Indobase services keep persistent connections to your database. For example, Storage, PostgREST, and our health checker all maintain long-lived connections. You usually see a small baseline of active connections from these services.
