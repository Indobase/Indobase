...
Any tables that are accessible through the Data API _must_ have [Row Level Security](/docs/guides/database/postgres/row-level-security) enabled. Row Level Security (RLS) is enabled by default when you create tables from the Indobase Dashboard. If you create a table using the SQL editor or your own SQL client or migration runner, you*must* enable RLS yourself.
...
- [Disabling the Data API entirely](#disabling-the-data-api). This is recommended if you _never_ need to access your database via Indobase client libraries or the REST and GraphQL endpoints.
...
You can disable the Data API entirely if you never intend to use the Indobase client libraries or the REST and GraphQL data endpoints. For example, if you only access your database via a direct connection on the server, disabling the Data API gives you the greatest layer of protection.

1. Go to [API Settings](/dashboard/project/_/settings/api) in the Indobase Dashboard.
...
Tables created via the Indobase Dashboard have RLS enabled by default. However, if you or your team create tables using the SQL editor, migrations, or an external tool, RLS will not be enabled automatically.
...
1. Go to [**Table Editor**](/dashboard/project/_/editor) in the Indobase Dashboard.
