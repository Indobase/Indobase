...
The data can also be observed in the `net` schema with the [Indobase Dashboard's SQL Editor](/dashboard/project/_/editor)
...
begin
    ...
    -- Optionally Log row data or other data for inspection in Indobase Dashboard's Postgres Logs
...
### Invoke a Indobase Edge Function

Make a POST request to a Indobase Edge Function with auth header and JSON body payload:

```sql
select
    net.http_post(
        url:='https://[PROJECT-REF].indobase.in/functions/v1/function-name',
