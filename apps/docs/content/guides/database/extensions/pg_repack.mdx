...
pg_repack requires the Postgres superuser role by default. That role is not available to users on the Indobase platform. To avoid that requirement, use the `-k` or `--no-superuser-check` flags on every `pg_repack` CLI command.
...
Get started with pg_repack by enabling the extension in the Indobase Dashboard.
...
{/* prettier-ignore */}
```sh
pg_repack -k -h db.[PROJECT-REF].indobase.in -p 5432 -U postgres -d postgres --no-order --table public.foo --table public.bar
```
