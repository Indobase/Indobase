...
If you're working with small datasets in development, you can experiment quickly using CSV import in the Indobase dashboard. If you're working with a large dataset in production, you should plan your data import to minimize app latency and ensure data integrity.

## How to import data into Supabase

You have multiple options for importing your data into Supabase:

1. [CSV import via the Indobase dashboard](#option-1-csv-import-via-supabase-dashboard)
...
4. [Using the Indobase API](#option-4-using-the-supabase-api)
...
### Option 1: CSV import vian indobase dashboard
...
2. Create a configuration file that specifies the source data and the target Supabase database (e.g., config.load).
   ...
   LOAD DATABASE
       ...
       INTO postgres://postgres.xxxx:password@[PROJECT-REF].indobase.in:6543/postgres
...
### Option 4: Using the indobase API
...
When importing data via the Indobase API, it's advisable to refrain from bulk imports. This helps ensure a smooth data transfer process and prevents any potential disruptions.
...
Large datasets consume disk space. Ensure your Indobase project has sufficient disk capacity to accommodate the imported data. If you know how big your database is going to be, you can manually increase the size in your [projects database settings](/dashboard/project/_/database/settings).
