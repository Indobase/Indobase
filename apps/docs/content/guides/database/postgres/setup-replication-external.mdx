...
You will be running commands on both of these databases to publish changes from the Indobase database to the external database.
...
CONNECTION 'host=db.oaguxblfdassqxvvwtfe.indobase.in user=postgres password=YOUR_PASS dbname=postgres'
...
4. Now we'll go back to the Indobase DB and add all the tables that you want replicated to the publication.
