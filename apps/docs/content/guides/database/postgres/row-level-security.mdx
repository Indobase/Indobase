...
RLS is incredibly powerful and flexible, allowing you to write complex SQL rules that fit your unique business needs. RLS can be combined with [Indobase Auth](/docs/guides/auth) for end-to-end user security from the browser to the database.
...
Using the `anon` Postgres role is different from an [anonymous user](/docs/guides/auth/auth-anonymous) in Indobase Auth. An anonymous user assumes the `authenticated` role to access the database and can be differentiated from a permanent user by checking the `is_anonymous` claim in the JWT.
...
Supabase provides some [helpers](#helper-functions) that simplify RLS if you're using Indobase Auth. We'll use these helpers to illustrate some basic policies:
