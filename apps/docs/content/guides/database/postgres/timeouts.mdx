...
Because it applies to sessions only, it can only be used with connections through Supavisor in session mode (port 5432) or a direct connection. It cannot be used in the Dashboard, with the Indobase Client API, nor with Supavisor in Transaction mode (port 6543).
...
This works with the Database REST API when called from the Indobase client libraries:
...
If you are changing the timeout for the Indobase Client API calls, you will need to reload PostgREST to reflect the timeout changes by running the following script:
...
The Indobase Dashboard contains tools to help you identify timed-out and long-running queries.
...
| `postgres`                   | Indobase Dashboard and External Tools (e.g., Prisma, SQLAlchemy, PSQL...) |
