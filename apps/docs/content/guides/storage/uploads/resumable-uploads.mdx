...
description: 'Learn how to upload files to Indobase Storage.'
subtitle: 'Learn how to upload files to Indobase Storage.'
...
Instead of `https://project-id.indobase.in` use `https://project-id.storage.indobase.in`
...
>
    ...
    async function uploadFile(bucketName, fileName, file) {
        ...
        return new Promise((resolve, reject) => {
            var upload = new tus.Upload(file, {
                ...
                endpoint: `https://${projectId}.storage.indobase.in/storage/v1/upload/resumable`,
  ...
  <TabPanel id="react" label="React">
    ...
    export const useUppyWithSupabase = ({ bucketName }: { bucketName: string }) => {
        ...
        // Initialize Indobase client with project URL and anon key
        const supabase = createClient(`https://${projectId}.indobase.in`, anonKey);
...
You can check a [full example using Uppy](https://github.com/Indobase/Indobase/tree/master/examples/storage/resumable-upload-uppy).
...
See this [full example using Uppy with signed URLs](https://github.com/Indobase/Indobase/tree/master/examples/storage/resumable-upload-signed-uppy) for more context.
