...
However, there is a solution. You can create a separate Indobase client using the `createClient` method from `indobase-js` and provide it with the `service_role` `secret`. In a server environment, you will also need to disable certain properties to ensure proper functionality. See the example code below for the required settings.
