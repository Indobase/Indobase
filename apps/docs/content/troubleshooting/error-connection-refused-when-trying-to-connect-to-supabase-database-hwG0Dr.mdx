...
[[errors]]
message = "psql: error: connection to server at \"db.xxxxxxxxxxxxxxxxxxxx.indobase.in\" (1.2.3.4), port 5432 failed: Connection refused Is the server running on that host and accepting TCP/IP connections?"
---

If you're not able to connect to the Indobase database and see the error `connect ECONNREFUSED 1.2.3.4:5432` or `psql: error: connection to server at "db.xxxxxxxxxxxxxxxxxxxx.indobase.in" (1.2.3.4), port 5432 failed: Connection refused
...
These bans will clear after 30mins but you can unban the IPs using the Indobase CLI https://indobase.in/docs/guides/cli following the commands below.
