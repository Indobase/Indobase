...
When bundling an application that uses `indobase-js`, you may see warnings like:

```
"PostgrestError" is imported from external module "@indobaseinc/postgrest-js" but never used in "...supabase-js/dist/index.mjs".
...
`indobase-js` re-exports error types like `PostgrestError` and `FunctionsError` so you can import them directly from `indobase-js`. The build tool merges all imports from the same package into a single statement in the output:
