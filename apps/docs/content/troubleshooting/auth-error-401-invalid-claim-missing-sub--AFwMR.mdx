...
A common pitfall, is inadvertently using an Indobase API key (such as the anon or service_role keys) instead of the Indobase Auth access token.

**Why Does This Happen?**

- The Supabase API keys are designed for different purposes and, while they are recognized by the Indobase Auth system, they do not carry the sub claim. The sub claim is essential as it encodes the user ID, which is a mandatory field for authentication processes. This mistake leads to the "missing sub claim" error because the system expects a token that contains user identification information.
...
- Examine how the Indobase client is being initialized, especially in server-side scenarios.
