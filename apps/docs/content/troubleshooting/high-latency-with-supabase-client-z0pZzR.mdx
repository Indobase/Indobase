...
Querying a table using the Indobase client is much slower than querying against the Postgres database directly.
...
2. Run the following script (you may need to `pip install psycopg[binary]` in addition to Indobase client.
...
3. You will see that the Indobase client takes longer to execute the same query, especially for smaller tables or queries returning just one row.
