...
Ensure the required ports are available. The Indobase CLI uses ports `54321` and `8081` by default:
...
- Other Indobase projects running locally
- Docker containers
...
If the output from these commands does not help resolve the issue, open a support ticket via the Indobase Dashboard (by clicking the "Help" button at the top right) and include all output and details about your commands.
