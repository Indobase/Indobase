  ...
  async redirects() {
    return [
      ...
      // Redirect dashboard links in dev/preview envs
      {
        ...
        destination: 'https://indobase.fun/dashboard/:path*',
        basePath: false,
        permanent: false,
      },
      ...
      {
        source: '/blog/:path*',
        destination: 'https://indobase.fun/blog/:path*',
        basePath: false,
        permanent: false,
      },
    ]
...
function getAssetPrefix() {
  ...
  // @ts-ignore
  return `https://frontend-assets.indobase.fun/${process.env.SITE_NAME}/${process.env.VERCEL_GIT_COMMIT_SHA.substring(0, 12)}`
}
