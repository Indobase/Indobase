...
<RefSubLayout.EducationRow>
  <RefSubLayout.Details>
    ...
    [PKCE flow](https://indobase.in/blog/supabase-auth-sso-pkce#introducing-pkce), which is a more secure method for obtaining sessions from deep links, is now the default auth flow for any authentication involving deep links.
