/**
 * Studio SSO — verifies handoff JWT from Studio (`aud=indobase-suite`, HS256).
 * Same contract as `apps/studio/lib/api/saas/product-handoff.ts`.
 */
import { createHmac, randomBytes, timingSafeEqual } from 'node:crypto'

export type SuiteRole = 'owner' | 'admin' | 'developer' | 'viewer'
...
export const AUDIENCE = 'indobase-suite'
export const SESSION_COOKIE = 'indobase_suite_session'
...
export function resolveHandoffSecret(): string {
  ...
  if (secret.length < 32) {
    throw new Error('SUITE_HANDOFF_SECRET missing or shorter than 32 chars')
  }
  return secret
}
