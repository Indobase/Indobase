...
- **V1.2 payments:** CheckoutService owns the payment **state machine** (pending → reserved → payment_pending → paid | payment_failed | expired). Duplicate webhooks and late success after expiry must not double-decrement stock. Do **not** add a Razorpay/Stripe agent tool.
