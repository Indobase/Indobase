...
describe('guest gates', () => {
  ...
  it('requires account for Launch / Ensure / mutate paths', () => {
    ...
    assert.equal(pathAllowsGuestRead('/api/os/tools/followups'), true)
    assert.equal(pathRequiresSignedInAccount('/api/os/tools/followups'), false)
    ...
    assert.ok(OS_ACCOUNT_REQUIRED_PATHS.includes('/api/os/apps/launch'))
    assert.ok(OS_ACCOUNT_REQUIRED_PATHS.includes('/api/os/tools/launchProductionApp'))
    assert.equal(pathRequiresSignedInAccount('/api/os/apps/launch/plj_test'), true)
