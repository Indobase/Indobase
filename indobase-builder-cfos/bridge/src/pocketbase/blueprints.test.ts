...
describe('backend blueprints', () => {
  ...
  it('ecommerce locks transactional collections to admin-only (Commerce authority)', () => {
    ...
    assert.equal(byName.product_variants.rules, 'public_read_admin_write')
    assert.equal(byName.catalog_collections.rules, 'public_read_admin_write')
    assert.ok(byName.product_variants.fields.some((f) => f.name === 'options_json'))
    assert.ok(byName.inventory_reservations.fields.some((f) => f.name === 'variant_id'))
    ...
    assert.ok(byName.orders.fields.some((f) => f.name === 'payment_state'))
