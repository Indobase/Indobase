...
export default function IndobaseBackendTab() {
  ...
  const isStudioManagedConnection = connection.connectionSource === 'studio_handoff';
  ...
  const openProjectDestination = (projectId: string, studioPath: string) => {
    if (isStudioManagedConnection) {
      ...
      if (studioRoot) {
        const normalizedPath = studioPath.replace(/^\//, '');
        const url = `${studioRoot.replace(/\/$/, '')}/${normalizedPath}`;
  ...
  // Project actions
  const projectActions: ProjectAction[] = isStudioManagedConnection
    ? [
        {
          name: 'Open Backend',
          icon: 'i-ph:layout',
          action: async (projectId: string) => openProjectDestination(projectId, 'backend'),
        },
        {
          name: 'Open Database',
          icon: 'i-ph:database',
          action: async (projectId: string) => openProjectDestination(projectId, 'database', '/editor'),
        },
        {
          name: 'Open Auth',
          icon: 'i-ph:user-circle',
          action: async (projectId: string) => openProjectDestination(projectId, 'auth/users', '/auth/users'),
        },
        {
          name: 'Open Storage',
          icon: 'i-ph:folder',
          action: async (projectId: string) => openProjectDestination(projectId, 'storage/files', '/storage/buckets'),
        },
        {
          name: 'Open Functions',
          icon: 'i-ph:code',
          action: async (projectId: string) => openProjectDestination(projectId, 'functions', '/functions'),
        },
        {
          name: 'Open Logs',
          icon: 'i-ph:scroll',
          action: async (projectId: string) => openProjectDestination(projectId, 'logs', '/logs'),
        },
        {
          name: 'Open API Keys',
          icon: 'i-ph:key',
          action: async (projectId: string) => openProjectDestination(projectId, 'settings/api-keys', '/api'),
        },
        {
          name: 'Open Settings',
          icon: 'i-ph:gear',
          action: async (projectId: string) => openProjectDestination(projectId, 'settings', '/settings'),
        },
      ]
    : [
        {
        ...
        },
        {
          name: 'Open API Docs',
          icon: 'i-ph:book',
          action: async (projectId: string) => openProjectDestination(projectId, 'api', '/api'),
        },
        {
          name: 'Open Realtime',
          icon: 'i-ph:radio',
          action: async (projectId: string) => openProjectDestination(projectId, 'realtime', '/realtime'),
        },
        {
          name: 'Open Edge Functions',
          icon: 'i-ph:terminal',
          action: async (projectId: string) => openProjectDestination(projectId, 'functions', '/functions'),
        },
      ];
  ...
  const handleConnect = async () => {
    if (!tokenInput) {
      toast.error('Please enter a backend access token');
      return;
    }
  ...
  const handleDisconnect = () => {
    ...
    toast.success('Disconnected from the backend');
  };
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-bolt-elements-textPrimary">
                {isStudioManagedConnection
                  ? `Indobase Projects (${connection.stats?.totalProjects || 0})`
          ...
          <div className="space-y-4 mt-4">
            ...
            {connection.stats?.projects?.length ? (
              <div className="mb-6 p-4 bg-bolt-elements-background-depth-1 rounded-lg border border-bolt-elements-borderColor">
                <h4 className="text-sm font-medium text-bolt-elements-textPrimary mb-3">Backend Overview</h4>
            ...
            {connection.stats?.projects?.length ? (
                        ...
                        {connection.credentials && (
                          <div className="bg-bolt-elements-background-depth-2 p-3 rounded-lg space-y-2">
                            ...
                            <div className="space-y-2">
                              <div>
                                <label className="text-xs text-bolt-elements-textSecondary">Backend URL:</label>
              ...
              <div className="text-sm text-bolt-elements-textSecondary flex items-center gap-2 p-4">
                ...
                {isStudioManagedConnection
                  ? 'No backend projects were available for this Indobase session'
                  : 'No projects found in your backend account'}
              </div>
            )}
          </div>
        </CollapsibleContent>
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {connection.user && !isStudioManagedConnection && (
            <Button
      ...
      <p className="text-sm text-bolt-elements-textSecondary dark:text-bolt-elements-textSecondary">
        {isStudioManagedConnection
          ? 'This Builder session is linked to your Indobase backend and can jump directly into Studio.'
          : 'Connect your backend account to manage projects, credentials, and Studio access.'}
      </p>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-bolt-elements-textSecondary flex items-center gap-1">
                    ...
                    {isStudioManagedConnection ? 'Connected to Indobase Backend' : 'Connected to backend'}
                  </span>
                </div>
              </div>
