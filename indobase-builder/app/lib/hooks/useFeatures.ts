...
export const useFeatures = () => {
  ...
  const acknowledgeFeature = async (featureId: string) => {
    try {
      ...
      const newViewedIds = [...new Set([...viewedFeatureIds, featureId])];
      const nextUnviewedFeatures = unviewedFeatures.filter((feature) => feature.id !== featureId);
      setViewedFeatureIds(newViewedIds);
      setViewedFeatures(newViewedIds);
      setUnviewedFeatures(nextUnviewedFeatures);
      setHasNewFeatures(nextUnviewedFeatures.length > 0);
    } catch (error) {
  ...
  const acknowledgeAllFeatures = async () => {
    try {
      ...
      const newViewedIds = [...new Set([...viewedFeatureIds, ...unviewedFeatures.map((f) => f.id)])];
      setViewedFeatureIds(newViewedIds);
      setViewedFeatures(newViewedIds);
      setUnviewedFeatures([]);
