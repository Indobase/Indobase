export function getIndobaseManagedBackendPrompt(options: {
  ...
  const { projectRef, apiUrl, anonKey, authUrl, storageUrl, restUrl } = options;
...
<indobase_managed_backend>
  ...
  API URL: ${apiUrl || 'provided in credentials'}
  Auth URL: ${authUrl || apiUrl || 'tenant auth endpoint'}
  REST URL: ${restUrl || apiUrl || 'tenant REST endpoint'}
  ...
  - VITE_INDOBASE_URL / NEXT_PUBLIC_INDOBASE_URL = ${apiUrl || 'tenant API URL'}
