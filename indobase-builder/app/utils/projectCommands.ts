...
function makeNonInteractive(command: string): string {
  ...
  const interactivePackages = [
    ...
    // Vite lives in devDependencies; omit --include=dev and `vite` is missing after restore.
    {
      pattern: /npm\s+install(?!\s+--)/g,
      replacement: 'npm install --yes --no-audit --no-fund --include=dev',
    },
...
export async function detectProjectCommands(files: FileContent[]): Promise<ProjectCommands> {
  ...
  if (hasFile('package.json')) {
    ...
    try {
      ...
      // Match template bootstrap: install with --include=dev so vite is present.
      let baseSetupCommand = 'npm install --yes --no-audit --no-fund --include=dev';
