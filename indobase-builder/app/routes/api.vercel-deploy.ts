...
type DeployEnvironmentVariables = Record<string, string>;

async function syncProjectEnvironmentVariables(options: {
  projectId: string;
  token: string;
  environmentVariables?: DeployEnvironmentVariables;
}) {
  const { projectId, token, environmentVariables } = options;
  ...
  for (const [key, value] of entries) {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to sync environment variable ${key}: ${errorData?.error?.message || errorData?.message || response.statusText}`,
      );
    }
  }
...
export async function action({ request }: ActionFunctionArgs) {
  try {
    const { projectId, files, sourceFiles, token, chatId, framework, environmentVariables } =
    ...
    const syncedEnvironmentVariables = await syncProjectEnvironmentVariables({
