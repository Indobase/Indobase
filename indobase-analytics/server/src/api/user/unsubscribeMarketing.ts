  ...
  try {
    ...
    if (!email) {
      if (isGetRequest) {
        return reply.status(400).type("text/html").send(`
          <!DOCTYPE html>
          <html><head><title>Unsubscribe - Indobase Analytics</title></head>
    ...
    if (isGetRequest) {
      return reply.status(200).type("text/html").send(`
        <!DOCTYPE html>
        <html><head><title>Unsubscribed - Indobase Analytics</title></head>
        <body style="font-family: sans-serif; max-width: 600px; margin: 50px auto; text-align: center;">
          <h1>Unsubscribed</h1>
          <p>You have been successfully unsubscribed from Indobase Analytics marketing emails.</p>
