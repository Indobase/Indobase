...
function StudioOnlySignupRedirect({ studioUrl }: { studioUrl: string }) {
  useEffect(() => {
        ...
        <CardContent>
          <div className="flex flex-col gap-6">
            <p className="text-center text-sm text-muted-foreground">
              Account registration is disabled. Open Analytics from Studio.
            </p>
            <a className="text-center text-sm underline text-primary" href={`${studioUrl}/sign-in`}>
              Continue to Studio
            </a>
          </div>
        </CardContent>
...
function SignupPageContent() {
  ...
  if (configs?.disableSignup) {
    ...
    return <StudioOnlySignupRedirect studioUrl={studioUrl} />;
  }
