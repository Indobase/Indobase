/**
 * Studio SSO — verifies handoff JWT from Studio (`aud=indobase-crm`, HS256).
...
export const AUDIENCE = 'indobase-crm'
export const SESSION_COOKIE = 'indobase_crm_session'
...
export function resolveHandoffSecret(): string {
  const secret = (
    process.env.CRM_HANDOFF_SECRET ||
    process.env.STUDIO_HANDOFF_SECRET ||
    ''
  ).trim()
  if (secret.length < 32) {
    throw new Error('CRM_HANDOFF_SECRET missing or shorter than 32 chars')
  }
  return secret
}
