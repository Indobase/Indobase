/**
 * Studio SSO — verifies handoff JWT from Studio (`aud=indobase-calendar`, HS256).
 * Same contract as `apps/studio/lib/api/saas/product-handoff.ts`.
 */
import { createHmac, randomBytes, timingSafeEqual } from 'node:crypto'

import { calendarRoleFromStudio, type CalendarProductRole, type StudioOrgRole } from './roles.js'
...
export const AUDIENCE = 'indobase-calendar'
export const SESSION_COOKIE = 'indobase_calendar_session'
...
export function resolveHandoffSecret(): string {
  ...
  if (secret.length < 32) {
    throw new Error('CALENDAR_HANDOFF_SECRET missing or shorter than 32 chars')
  }
  return secret
}
...
export function readSessionToken(value: string, secret: string): Session | null {
  ...
  try {
    ...
    const role = p.role as StudioOrgRole
    if (!['owner', 'admin', 'developer', 'viewer'].includes(role)) return null
    const mapped = calendarRoleFromStudio(role)
    const studioUrl =
    ...
    return {
      ...
      role,
      calendarRole: mapped.calendarRole,
      canManage: mapped.canManage,
      canEdit: mapped.canEdit,
      studioUrl,
    }
  } catch {
    return null
