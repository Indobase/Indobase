# Single-domain deployment (indobase.in)

Deploy the **marketing site**, **Studio**, and **backend** on one domain: **indobase.in**.
...
- `PUBLIC_APPWRITE_DASHBOARD` = `https://indobase.in/dashboard` (Studio URL).
- `PUBLIC_APPWRITE_ENDPOINT` = `https://indobase.in/api/v1` (backend API, if the site calls it).
...
- `NEXT_PUBLIC_SITE_URL` = `https://indobase.in/dashboard` (Studio’s public URL).
- `NEXT_PUBLIC_DOCS_URL` = `https://indobase.in/docs`.
- Backend vars (`SUPABASE_URL`, `NEXT_PUBLIC_GOTRUE_URL`, etc.) should point to the same domain, e.g.:
  - `NEXT_PUBLIC_GOTRUE_URL` = `https://indobase.in/auth/v1`
  - Platform API: `https://indobase.in/api` (or whatever path your gateway exposes).
...
- In **supabase/config.toml** → `[auth]`:
  ...
  - **additional_redirect_urls** already include `https://indobase.in/*` and `https://indobase.in/*/*` for single-domain.

- Kong (or your gateway) in front of GoTrue/PostgREST/etc. should listen on the same domain and route, e.g.:
  - `https://indobase.in/auth/v1` → GoTrue
  - `https://indobase.in/rest` → PostgREST
  - `https://indobase.in/api` → your platform API (if any)

## Summary

- **One domain:** `https://indobase.in`.
- **Marketing:** `/` (and the paths it handles).
- **Studio:** `/dashboard` with **NEXT_PUBLIC_BASE_PATH=/dashboard** and **NEXT_PUBLIC_SITE_URL=https://indobase.in/dashboard**.
- **Backend:** under `/api`, `/auth/v1`, `/rest`, etc., via reverse proxy; auth redirect URLs in config point at `https://indobase.in/*`.
