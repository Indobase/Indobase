...
type Config struct {
	...
	// Indobase Studio SSO (operators sign in via Studio, not magic-code / password).
	StudioHandoffSecret string // shared HMAC secret with Studio (EMAIL_HANDOFF_SECRET)
	StudioPublicURL     string // e.g. https://studio.indobase.in — redirects for killed public auth
	StudioHandoffOnly   bool   // when true, disable public magic-code / root HMAC sign-in
...
func LoadWithOptions(opts LoadOptions) (*Config, error) {
	...
	v.SetDefault("DB_PREFIX", "indobase_email")
	v.SetDefault("DB_NAME", "indobase_email_system")
	...
	config := &Config{
		...
		StudioHandoffSecret: firstNonEmpty(
			v.GetString("STUDIO_HANDOFF_SECRET"),
			v.GetString("EMAIL_HANDOFF_SECRET"),
			v.GetString("PAYMENTS_HANDOFF_SECRET"),
		),
		StudioPublicURL:   strings.TrimRight(v.GetString("STUDIO_PUBLIC_URL"), "/"),
		StudioHandoffOnly: v.GetBool("STUDIO_HANDOFF_ONLY"),
