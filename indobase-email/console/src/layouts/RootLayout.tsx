...
export function RootLayout() {
  ...
  const isLaunchRoute = useMatch({ from: '/console/launch', shouldThrow: false })
  ...
  const isPublicRoute =
    isSigninRoute || isLaunchRoute || isAcceptInvitationRoute || isLogoutRoute || isSetupRoute

  // If system is not installed, redirect to setup wizard
  const shouldRedirectToSetup = !isInstalled && !isSetupRoute

  // If not authenticated and not on public routes, redirect to Studio (via signin redirect)
