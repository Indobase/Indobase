/**
 * Studio SSO — verifies handoff JWT from Studio (`aud=indobase-meet`, HS256).
 * Same contract as `apps/studio/lib/api/saas/product-handoff.ts`.
 */
import { createHmac, randomBytes, timingSafeEqual } from 'node:crypto'

import { meetRoleFromStudio, type MeetProductRole, type StudioOrgRole } from './roles.js'
...
export const AUDIENCE = 'indobase-meet'
export const SESSION_COOKIE = 'indobase_meet_session'
...
export function resolveHandoffSecret(): string {
  ...
  if (secret.length < 32) {
    throw new Error('MEET_HANDOFF_SECRET missing or shorter than 32 chars')
  }
  return secret
}
...
export function readSessionToken(value: string, secret: string): Session | null {
  ...
  try {
    ...
    if (!['owner', 'admin', 'developer', 'viewer'].includes(role)) return null
    const mapped = meetRoleFromStudio(role)
    const studioUrl =
    ...
    return {
      ...
      role,
      meetRole: mapped.meetRole,
      isModerator: mapped.isModerator,
      studioUrl,
    }
  } catch {
    return null
